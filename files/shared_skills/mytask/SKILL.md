...
- 着手する項目を `in_progress`、外部依存で進められない項目を `blocked`、未着手を `pending` にする。成果物と完了条件を検証してから `completed` にする。
- 大きな区切りや計画変更時には、依頼、現在の到達点、次の作業を短く照合する。説明できなければ元の依頼と項目を読み直す。
- 初回の計画と重要な変更、完了・保留・残作業をユーザーに分かる言葉で示す。MCP の一覧が画面に自動表示されるとは限らないので、本文にも要約する。機械的に全項目を毎回転記しない。
...
- `TaskList()`：現在の項目を読む。
- `TaskCreate(content, parent?, activeForm?)`：依頼や手順を登録する。子項目には親の id を `parent` に渡す。
