...
def _bump(path: str, now: int) -> tuple[int, int]:
...
def _emit_turn_marker(payload: dict) -> None:
...
def main() -> int:
    try:
        ...
        return 0
    try:
        exit_code = _run(payload)
    except Exception:
        exit_code = 0  # fail-open: an enforcement glitch never blocks the turn
