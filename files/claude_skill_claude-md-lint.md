...
スキル実行のたびに対象ファイルを **必ず Read tool で読み直す**。 前のターンの context、claudeMd block、user message に inline された内容には依存しない（セッション中にファイルが編集される、または前回のキャッシュが古い可能性があるため）。

### ファイル解決アルゴリズム

cwd は環境のプライマリ working directory。 以下を順に試し、存在するものを入力に加える。
...
各ファイル本文から `@<path>` 参照を抽出し、BFS で深さ 5 まで再帰的に Read する。
...
SessionStart hook から user message に inline content が渡された場合でも、その content は **参考にせずディスクから読み直す**（drift 防止）。 ただし hook が cache key 計算のため inline するのは hook 側の責務なので、skill としては inline の有無に依存しない。
