...
- 冗長な処理、過剰 retry、巨大 output、不要な全体 Read、繰り返しの全文 dump を避けるよう最大限の注視する
- Code comment は ** 1 行以内** 、非常に困難な場合のみ 2 行を許可、3 行以上は厳禁。 Code が主役であり、Comment は補助具で人間の code reading と debug 加速のためだけにある。 補助具に token を浪費するな。 既存コメントが長いことは言い訳にならない
...
- 報告では、判定・推奨・結論・規模影響評価を発話する前に、公式情報 / コード / 文書 / 設定 / memory entry 本文 などを必要範囲で読み、根拠として示す。読んでいなければ「未確認」と明言する。「不明」「該当なし」は確認後にのみ結論する。読んだ後は影響ファイル数・節・呼び出し元など定量表現で述べる

# スキルの活用

スキルはユーザーとのコミュニケーションを円滑にするノウハウ集。 スキルを軽視せずあらゆる場面で invoke すれば、 friction が減ってユーザーの評価が高まる。 作業のやり直しが減り Token 効率も向上する。 多くの Write / Edit tool 利用では、スキルを経由しないと hook deny されるので気をつける。
