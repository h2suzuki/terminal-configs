...
- Code comment は **原則 1 行**、 非常に困難な場合のみ 2 行まで (3 行以上は禁止)。 主役は code、コメントは補助具。 その目的は人間の code reading と debug 加速。 補助具に token 浪費は厳禁。 既存コメント (自分が過去に書いた分を含む) が長いことは長文化の言い訳にならない
...
スキルにはユーザーとのコミュニケーションを円滑にするノウハウが詰まっている。 スキルを活用できるあらゆる場面で invoke することで、friction が減り、ユーザーの評価が高まる。作業のやり直しが減り Token 効率も向上する。 Write / Edit tool 利用時は、スキルを経由しないと失敗する hook gate もあるので注意する
