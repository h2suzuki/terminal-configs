...
(永続ファイルから dangling-prone reference [端末固有 path / skill dir 外 file への file path citation / ephemeral tag 等] を入れない — `code-conventions` Rules「No dangling-prone references in persistent files」 参照)
