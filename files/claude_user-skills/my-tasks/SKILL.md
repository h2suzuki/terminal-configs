...
5. 作業一覧の報告は id 昇順で各項目を `<id> <emoji> <content> (<created>)` 形式で出力し、サブタスク (`1-1` 等) は親の下に 2 space 字下げする。emoji は status に対応: 新規(`pending`)=🔳 / 作業中(`in_progress`)=🔛 / ブロック(`blocked`)=🚧 / 完了(`completed`)=✅。
