...
- **Model-scoped replay**: rescore / misses は event 記録時の model (`events.jsonl` の `model`、misses は transcript から抽出) で filter 込み replay する。model 列導入前の旧 event は `model=None` = 無 filter で従来どおり。`kind='mismatch'` の event は live で inject されなかった would-be hit なので precision 集計に混ぜず、tag 追記候補の材料として別枠で読む。
