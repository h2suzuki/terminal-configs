#!/usr/bin/env python3
"""Orderer-owned mutants for memory_routing_gate.py: each must make the acceptance tests fail.
...
MUTANTS = {
    "m1-cap-boundary-off-by-one": (
        "ORG_CAP = 60",
        "ORG_CAP = 59",
    ),
    "m2-rewrite-treated-as-new": (
        "    return not os.path.exists(path)",
        "    return True",
    ),
    "m3-never-list-dropped": (
        "NEVER_SHOWN = 15",
        "NEVER_SHOWN = 0",
    ),
    "m4-cap-applied-to-every-scope": (
