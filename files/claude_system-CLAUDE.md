...
- skipped (test skip / verification step skip) を completed と報告しない。「学習した」「次回から気をつける」「もう間違えない」「反省」系の発言は、memory file 更新などの persistence 行動とセットの時のみ行う。session 境界を越えると虚偽になる。
...
### 3. 完了前に検証
- 動作を証明できたタスクのみ完了とマークする。テストを実行し、ログを確認し、正しさを示す。
...
- 「動かない」「効かない」「失敗した」現象に出会ったら、まず原因仮説を 2-3 立てて、コード / log で裏付けに行く → artifact-level の修正提案、の順で根本に着地させる（symptom 緩和の workaround: 手動 spawn / session 再起動 / 再 cd 等を勧めない）。
