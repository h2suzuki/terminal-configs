...
| 開発マシン以外への配備 (デプロイ) の予定・進み具合 | どこにも書かない。 どのマシンにいつ配るかはユーザーの管轄で、 追跡も催促もしない。 変更を開発マシンに配備して実際に動かすテストは作業の一部なので、 これまでどおり省略しない |
...
- 決裁 / 承認 / 合意 / 採用 / 保留 / 判断 を含む項目には「…」のユーザー発話の引用か「提案中」等の非決定 marker を添える。 ユーザーが言っていない「保留」「判断待ち」を書かない
