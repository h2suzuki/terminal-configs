...
3. **実装は codex へ委譲**: `/codex:rescue <spec>` で codex に渡す (長時間は `--background`、 進捗 `/codex:status`、 結果 `/codex:result`、 中断 `/codex:cancel`)。 前回 run の継続 (apply top fix / 深掘り) は `--resume`、 仕切り直しは `--fresh`。 spec に達成条件・制約・対象 file・受入基準を載せる。
