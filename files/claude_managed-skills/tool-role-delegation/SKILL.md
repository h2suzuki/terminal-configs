...
3. **実装は codex へ委譲**: `/codex:rescue <spec>` で codex に渡す (長時間は `--background`、 進捗 `/codex:status`、 結果 `/codex:result`)。 codex を MCP server 登録済なら `mcp__codex__codex` 直呼びも可 (cwd / sandbox / approval-policy 指定)。 spec に達成条件・制約・対象 file・受入基準を載せる。
...
検索は codegraph の適切なツール、 実装は `/codex:rescue` (または `mcp__codex__codex`) 委譲 → Claude レビュー、 高リスクは `/codex:adversarial-review` 追加。 委譲不可時のみ Claude が直接。
