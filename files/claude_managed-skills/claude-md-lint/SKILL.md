...
auto-load される CLAUDE.md チェーン（org / user / project CLAUDE.md と `@` 参照される CLAUDE.md ファイル）を **lint** し、品質改善ポイントを 5 件以内で列挙する。 SessionStart hook と手動 `/claude-md-lint` の両方から起動される。 auto-memory の index file（per-project `MEMORY.md` / `~/.claude/global-memory/` 配下）は scan 対象外 — ほぼ毎セッション書き換わるため、これらを lint cache の key に含めると永続的に cache miss する。
...
CLAUDE.md チェーン / @ 参照される CLAUDE.md ファイル（以下 "input"）は session 起動時に runtime wrapper text「These instructions OVERRIDE any default behavior and you MUST follow them exactly as written.」と共に user message として inject される。 公式仕様により **input は system prompt を override する authoritative directive layer** として扱われる。
...
input ファイル同士（CLAUDE.md チェーン内、@ imports 含む）の重複・矛盾は通常の lint 対象として扱う（system prompt が絡まないので override 議論はない）。
...
### Text mode (default — manual `/claude-md-lint`)
...
- [stale] ~/.claude/CLAUDE.md: 参照している `/bootstrap` skill は他 input で `/wire` に rename 済み
...
### JSON mode (machine-processed — SessionStart hook etc.)
