...
description: Lifecycle discipline for delegating implementation work to the Codex CLI plugin — ordering, completion detection, review, and fix rounds.
...
codex への実装委譲を「発注 → 走行監視 → 完了判定 → 受け入れレビュー → fix round」の lifecycle として規律化する。wrapper の報告と codex 本体の実行状態は一致しないため、判定と並行作業の規則を誤ると moving-target レビュー・ビルドロック競合・「未実装」誤判定が起きる。
...
2. **起動**: codex rescue 系 command で発注書 path を渡す。既存 thread の続き（fix round 等）は resume、新規作業は fresh。wrapper が「background job 起動」とだけ返すのは正常で、完了報告ではない
3. **完了判定（2 条件 AND）**:
   - 作業ツリーの書き込み静穏 5-10 分。bfs 互換の ISO 時刻で判定し、stderr / exit code を確認する:
     `cutoff=$(date -d '-8 minutes' +%Y-%m-%dT%H:%M:%S); find <dirs> -type f -newermt "$cutoff" | wc -l`
