...
2. **発注書を書く**: 骨組みの空欄を埋める。依頼は chat 文でなく発注書 file（作業 dir の drafts/ 等）に固定する。**発注書が参照する gitignore 配下の資料（main checkout の `drafts/` の設計書等）は絶対 path で書く** — worktree は tracked file しか共有せず、かつ repo 外に在るので、相対 path は worktree 内で不存在になり BLOCKED 終了する（60〜120 分見積もりの task が 5 分で completed になった 2026-07-30 実例。codex は worktree 外の絶対 path を読めるので複製は要らない）。含める: スコープ（触ってよい path / 触らない path）、仕様の優先順位（受け入れ修正節が本文に優先する等の明文）、完了条件（fmt / clippy / test の実行と **結果ログの file 保存**）、「コミットはしない（受け入れレビュー後に発注側が行う）」の明記
...
3. **起動**: 発注は **codex:rescue skill 経由のみ** — model が companion (`codex-companion.mjs`) を Bash で直接起動する形は全 subcommand が gate で deny される（2026-08-21 ユーザー決裁。解除はユーザーが作る承認 file のみ）。rescue の raw request に実行 flag（`--fresh` / `--resume`・`--background`）、runtime flag（`--write`・`--model` 正式 id・`--effort`）、短文 prompt（発注書 path 1 つ + 第一動作 + 終端 token。報告書 path は発注書側に書き、prompt に `.md` path を 2 つ以上入れない — delegation gate の `[order]` 規則は command 中の `.md` path を数えて write task に「ちょうど 1 つ」を要求する。報告書 path も書いた prompt が deny され、転送 subagent が 15 tool 呼出の試行錯誤に陥った実例 2026-08-27）、作業 dir（手動 worktree の絶対 path）を渡す。**実装発注は `--write` 必須（既定 = read-only sandbox）**。発注 prompt の第一動作に write probe file 作成を入れ、起動 1-2 分後に実在を確認する（数十分の空走を早期検知）。既存 thread の続き（fix round 等）は resume、新規作業は fresh。wrapper が「background job 起動」とだけ返すのは正常で、完了報告ではない
   ...
   - **起動 command 行に長い日本語 prompt を直書きしない**: auto-mode classifier が長い日本語のコマンド行を確率的に deny する（別 project で 5 回中 3 回 deny の実測 2026-08-12〜13。短い機械的コマンド行は通過する対照実験済み）。prompt は「発注書 path + 第一動作 + 終端 token 確認」程度の短文に留め（報告書 path は含めない）、長い指示は発注書 file 側に書く。この注意は rescue の raw request に渡す prompt 文にも適用される（forwarder が組む task コマンド行に乗るため）。過去の「exclusion でホスト実行されるから直接起動でよい」運用と専用 launcher 案は、直接起動の全面禁止（2026-08-21）でいずれも廃止
...
- **worktree の命名（推奨）**: `~/worktrees/<repo>/<name>` の `<name>` はブランチ名、または GitHub issue 番号に対応する `issue-123` などを推奨する。命名は強制・自動検証しない。ブランチ名の `/` をそのまま使う場合、`feature/foo` は `~/worktrees/<repo>/feature/foo` という階層になる。
...
- **実装委譲は手動 git worktree で起動する**: `task --write` は発注側が `git worktree add`（**置き場は repo 外の `~/worktrees/<repo>/<name>` 固定**・ライフサイクルは発注側管理）で作成した worktree を cwd にして起動し、発注 session も発注前に単独の `cd` でその worktree へ移って終了まで留まる（cd せずに発注しない — 上の broker 後始末の鍵ずれ）。Agent `isolation: "worktree"` は使わない — harness の unchanged 自動掃除が走行中の codex の worktree を削除した実例（probe 書込先 drafts/ が gitignore 下で unchanged 判定・2026-07-21）。監視用 babysitter agent は非隔離で spawn し worktree へ cd して運用する。`--write` なしの read-only review task は隔離しなくてよい。main worktree への委譲で別 session の変更 14 file が堆積し、うち `main.rs` は行レベルで双方の追加行が混在した（2026-07-20）。**repo 内に置かない** — 実体は untracked dir として `git status` に残り、grep / lint / find が二重に走り、掃除の巻き添えで一次資料ごと消える（`wt-gates` が 2 週間放置され untracked のまま発見・2026-09-05 ユーザー指示で外出し。worktree ごと消して r77–r82 の一次資料を失った先例あり）。`~/worktrees` は managed settings の `sandbox.filesystem.allowWrite` に登録する — sandbox は session 開始後に作られた dir だけ素通しし、開始時に在った dir は deny する（`~/.cache` deny / 作成直後の `~/worktrees` OK を実測・2026-09-05）ため、登録が無いと作成した session でだけ書けて次 session で書けなくなる。**repo 外へ出したぶん codex の trust 登録が要る** — `~/.codex/config.toml` の `[projects."<絶対path>"] trust_level = "trusted"` に無い作業 dir で codex は trust 画面を出し、`approval_policy = "never"` や `sandbox_mode` の明示では抑止されない (openai/codex#14547 の 0.113.0 regression。当環境 0.153.2)。wildcard `[projects."*"]` は効かず絶対 path のみ受理される。`~/worktrees` を 1 度登録すれば配下は継承する (repo 内 worktree が repo root の trust を継承して動いていた実績と同じ機序・2026-09-05 監査)
