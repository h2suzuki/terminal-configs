...
2. **外部ソースを順に読み直す**:
   - **mytask** (mytask MCP の TaskList): 現在のタスク state と pending 承認待ち事項
   - `todos.md` がもしあれば: セッション跨ぎの大項目
...
5. **中断する場合**: 現在の作業を脇に置いて別作業へ切り替える場合は、 中断することをその経緯と共に mytask に必ず登録する。 再開したら、 この項目は `completed` で閉じて良い。
