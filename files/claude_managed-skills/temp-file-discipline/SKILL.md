...
limited to the directory that invocation created. Small, short-lived temp may go in the
scratchpad directory the harness names for this session
(`/tmp/claude-<uid>/<project>/<session ID>/scratchpad/`), the only `/tmp` path the
SessionEnd hook removes; nothing of uncertain or large size goes in `/tmp`. Do not rely
on automatic deletion of drafts or on another client's cleanup.
