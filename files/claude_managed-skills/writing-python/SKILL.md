...
- **post-edit check は inline 実行で workflow 化しない**（決定的 oracle ゆえ。 universal な扱いは `writing-code`）。 ruff / ty は dir・project 単位で native batch するので repo 全体でも 1 プロセスで足り、 per-file で agent を fan out する必要がない。 抑制 token（`# noqa` / `# type: ignore`）は誤検知が確実な箇所に限り理由を添える。
