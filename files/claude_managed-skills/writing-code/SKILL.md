...
コメントは **原則 1 行**。 1 行で表現するのが**非常に困難な場合に限り 2 行まで**、 3 行以上は書かない。 system prompt の "Never write multi-paragraph docstrings or multi-line comment blocks — one short line max" を踏襲。

- 「原則」は逸脱を緩く許す default ではない。 2 行を使うのは 1 行への圧縮を実際に試みて尚不能な時だけ。 まず削れないか・identifier 改名で WHY が自明にならないかを先に検討する
- 既存コメント (自分が過去 / 同 session に書いたものを含む) が長いことは、 新しいコメントを長く書く正当化に**ならない**。 長い既存を見つけても踏襲せず、 短縮対象として扱う
