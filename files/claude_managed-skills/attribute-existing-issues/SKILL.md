---
name: attribute-existing-issues
description: In persisted text (commit message body / memory entry / SKILL.md / CLAUDE.md / doc), do not blur the current session's contribution by attributing a wrong pattern the session inherited or extended to phrasing like "existing" / "carried over" / "reasonable default" / "incremental extension" / "didn't notice"; write the honest framing that names the session's own action against the pre-existing pattern.
...
- `report-by-evidence` — 判定 / 推奨を発話する前に根拠を示す。 本 skill の 「私の判断は X、 根拠は Y (出典 Z)」 と補完
