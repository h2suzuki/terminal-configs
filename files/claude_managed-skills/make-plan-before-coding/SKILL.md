...
5. **Claude Code 機能** (hook event / skill frontmatter / `claude --bg` 等 hidden flag / settings.json schema / MCP / plugin) を含む planning なら、 `${XDG_CACHE_HOME:-~/.cache}/claude-code-feature-cache/findings.md` を Read して cutoff 以降の delta を check。 最上位 `## v<X.Y.Z>` heading と `claude --version` を突き合わせ、 match していれば既知範囲は cache が cover、 不一致 / 不在なら bg dispatch 中なので既知範囲で進めつつ negation / positive 断定は後追い verify
