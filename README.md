...
| <div align="right">ツール設定</div> | `/etc/claude-code`、`/etc/codex`、Sandbox 設定、Claude Code 標準の Task ツールの無効化（作業の記録は mytask に一本化）など |
...
ChatGPT アプリの「リモート」で「接続を追加」を選び、「手動でペアリング」に `pair` が表示するコード（`XXXX-XXXX`）を入力します。
