...
インストール後、Claude Code のコンソールで `/mcp` と `/doctor` を実行して OAuth2 認証を済ませてください。Toolbox（BigQuery）の利用には gcloud 側の設定も必要です（`gcloud config set project <プロジェクトID>` と `gcloud auth application-default login`）。
