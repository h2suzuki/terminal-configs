...
CANONICAL="${VVOX_SCRIPT:-$(dirname "$(realpath "$0")")/../files/voicevox_claude_alerts}"
...
# CASE 7: VVOX_LOCK held → bounded `flock -w` drops the utterance instead of
# queuing a detached waiter (guards the unbounded-pileup accumulation bug).
CUR="VVOX_LOCK contended → flock -w drops utterance, no pileup"
echo "=== $CUR ==="
...
# hang) is SIGKILLed by `timeout -k`, so it cannot linger holding the lock.
CUR="stuck SIGTERM-deaf paplay → timeout -k SIGKILLs it, no linger"
echo "=== $CUR ==="
STUCK_FIFO="$SB/stuck.fifo"; mkfifo "$STUCK_FIFO" 2>/dev/null
cat > "$BIN/voicevox_paplay" <<STUCK_EOF
#!/usr/bin/env bash
echo "\$*" >> "$VVOX_LOG"
trap '' TERM
read -r _ < "$STUCK_FIFO"
STUCK_EOF
chmod +x "$BIN/voicevox_paplay"
reset_run
printf '%s' '{"hook_event_name":"SubagentStart","session_id":"s8","agent_type":"general-purpose"}' \
  | VVOX_TIMEOUT=1 VVOX_LOCK_WAIT=10 bash "$SCRIPT" SubagentStart >/dev/null 2>&1
for _ in $(seq 1 30); do [ "$(vvox_count)" -ge 1 ] && break; sleep 0.1; done
GONE=0
for _ in $(seq 1 100); do pgrep -f "$BIN/voicevox_paplay" >/dev/null 2>&1 || { GONE=1; break; }; sleep 0.1; done
if [ "$(vvox_count)" -ge 1 ] && [ "$GONE" = 1 ]; then ok "$CUR"
else bad "$CUR (vvox=$(vvox_count) gone=$GONE; stuck paplay must be SIGKILLed within timeout+grace)"; fi
pkill -KILL -f "$BIN/voicevox_paplay" 2>/dev/null
