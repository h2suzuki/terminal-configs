...
3. `when_to_use` — 発火条件。 `TRIGGER when ...` は必須です。 `SKIP when / for ...` は skip すべき条件が実在するときに書きます (常に適用される skill では省略してかまいません)。 trigger に埋め込むキーワードは、 引用符付きの日本語 (例 `"網羅した"`) を許容します。 この列挙では具体フレーズを引用します。 一般化しすぎると発火しません。
