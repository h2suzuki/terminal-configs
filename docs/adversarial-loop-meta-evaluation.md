...
- 実測 (2026-08-26): ケース 1 = sentinel 書き直しは 1 巡で出荷。ケース 2 = todos 構造 gate は
  ...
  する) を P0 として出し、契約の訂正 → 再入場 1 回で受け入れ。両ケースとも変異 0/4
- 判定 (2026-08-28): **不成立**。stop_checks が配備翌日に用途内 P0 を 2 件出した — skill 再 invoke を
  turn 境界と誤認 / 引用された `<task-notification>` で workflow gate が開く。2 件とも契約 C2 / C8 自体の
  誤りで、契約から作った受け入れ test も固定 4 変異もその誤りを再現するだけ。hook 間の噛み合わせ
  (skill_reminder_gate が強制する invoke が偽境界を作る) は context 遮断の独立レビューにも見えない。
  事前宣言どおり loop approach を捨て、規約は巡と再入場を削除して 5 項目にした
