# codex_task_sentinel 敵対レビュー 17〜52 巡 集計

`files/codex_task_sentinel` に対する 36 巡 (17〜52 巡) の敵対レビューで挙がった 174 件の指摘を、
処理ブロック・原因タイプ・由来 (self / preexisting / unknown) で集計した文書である。
復元元は 報告書原文 (r17〜r30 と r52 のみ現存)、発注書 r17〜r52 の「## 目的」、commit log、および transcript。
...
36 巡・**174 件**。件数の推移 (r19〜r51 は r52 発注書の推移行、r17 / r18 は r18 / r19 発注書の本文、r52 は報告書原文から):

```
r17..r52: 5 10 12 5 5 4 4 4 5 3 6 4 5 8 5 5 9 7 6 6 4 5 2 3 2 3 2 3 3 5 3 4 4 4 3 6
```
...
| 46–52 | 5 / 3 / 4 / 4 / 4 / 3 / 6 | 29 | 25 | 1 | 3 | 86% |
| **計** | | **174** | **109** | **46** | **19** | **63%** |
...
- **「preexisting は 39 巡で尽きた」という 51 巡時点の読みは、52 巡で覆った**。r52 指摘 6 (`str.splitlines()` が
  ...
  **30 commit 前** — レビュー開始初期から在って 50 巡以上素通しされた欠陥である。40〜51 巡に preexisting が
  ...
  ためと読むほうが資料と整合する。
- **self 率は単調に上がり続け、46〜52 巡では 86%**。件数自体は 12 (r19) → 2〜6 に収束したが、
...
| **計** | **174** | **81** | **35** | **34** | **12** | **12** |

ブロックごとの偏り:

| 観察 | 数字 |
|---|---|
| 上位 3 ブロック (登録解決 / record 読取 / 表示) で全体の **44%** | 77 / 174 |
| 「同じ規律を全 site へ配る」型 (forgot) が支配的なブロック: 登録解決・record 読取・ツリー・CLI引数 | 各ブロックの 47〜100% |
| 「自分の裁定 / docstring / comment と code が食い違う」型 (contradictory) が支配的: 判定 78%、表示 33%、成果物 33% | 判定 7/9、表示 7/21、成果物 7/21 |
...
| 原因タイプ | 件数 | 比率 | 一言でいうと |
|---|---:|---:|---|
| forgot | 81 | 47% | 同種の site の一部にしか配っていない |
| imagined | 35 | 20% | 現実に対応しない前提・到達不能な分岐・呼ばれない関数を patch した test |
| contradictory | 34 | 20% | 自分で書いた裁定 / comment / docstring と code が矛盾 |
...
### open (5 クラス) — 52 巡時点で未収束

52 巡の 6 件は **すべてこの 5 クラスのいずれか**に収まった。新しいクラスは 1 つも増えていない。

| クラス | 件数 | 出現巡 | 状況 |
|---|---:|---|---|
| 登録解決 / forgot | 16 | 19,27,28,29,30,31,32,33,47,48,51,52 | 全クラス中最多。complete 戻り値・seen_pairs・走査上限・本文 id・掴みの記憶と毎巡 1 段ずつしか配れていない。r52 指摘 3 (`resolved_unknown` が読めただけで消える) を `6dcb92e` で閉じたが **test で守れていない** |
| 期限 / unfinished | 7 | 21,33,36,37,48,49,52 | gate 外の return。r52 指摘 1 (`24ac731`) で 7 件目。48 巡と同じ形を 4 巡後に再演 |
| 成果物 / imagined | 6 | 24,27,28,44,52,52 | FIFO で到達しない guard / ABA / queued job が成果物を書く前提 / inode 番号再利用。r52 は指摘 5 (読取窓だけの消失を stable missing に畳む) と指摘 6 (`splitlines` の行末認識) の 2 件 |
| 登録解決 / unfinished | 4 | 36,47,50,52 | gate を通らない早期 return (36/47) と descriptor の解放順序 (50)。r52 指摘 2 を `6dcb92e` で閉じたが **test で守れていない** |
...
| 成果物 / forgot | 8 | 17,18,19,22,30,39,46 | 3 値化・bounded read・strict decode・descriptor 指紋・hold。log / record へ先に配った対策が成果物にだけ届かない |
...
| CLI引数 / forgot | 3 | 19,30,38 | option 組合せ / 巨大値の OverflowError / job id の path component 未検証 |
...
判定できるのは r17〜r51 の 35 巡 (r52 の効果は 53 巡の結果が出るまで未判定) で、
**worsened 33 / mixed 2 (r30, r43) / improved 0**。
mixed の 2 巡は「次巡の指摘に由来の記載が無く帰属できない」巡であり、改善が確認できた巡ではない。

修正が次巡に生んだ件数の合計は **90 件** (174 件の 52%)。
...
### レビューを待たずに自分で見つけた指摘 (14 巡・計 20 件)
...
### 52 巡目の 6 件 — 全件 commit 済 (うち 2 件は test で守れていない)

| 指摘 | 内容 | ブロック / 原因 | 由来 | 状態 |
|---|---|---|---|---|
| 1 | 周回間の record 差し替え分岐だけが deadline gate を迂回し、期限後に exit 14 | 期限 / unfinished | self (r51 `8b43dea`) | `24ac731` 変異検証済 |
| 2 | 読めない周回で「新しい掴み」が無いのに古い record descriptor を手放し、後発 inode を比較不能にする | 登録解決 / unfinished | self (pin 系 r44–51) | `6dcb92e` **test 無し** |
| 3 | `resolved_unknown` が次周で読めただけで消え、未検証候補を後から別 record へ置き換えられる | 登録解決 / forgot | self (r51 `968714f`) | `6dcb92e` **test 無し** |
...
- **r52 指摘 2・3 (掴みの持ち越しと `resolved_unknown` の非クリア)**: 差の出る fixture を作れなかった。
  1 周目の unknown がその場で exit 13 になるため、2 周目が来る入力を構成できない。
  緑のまま何も守らない test を残すより削除を選び、`6dcb92e` の commit message に欠落として記録した。
  **51 巡でも同じ箇所が同じ理由で test 無しのまま閉じられ、52 巡で「不十分」と再判定されている** — 2 巡連続の同型。
...
| 報告書原文 | `/home/h2suzuki/terminal-configs/drafts/sentinel-review-r52-report.md` | r52 のみ現存 (52 巡目から drafts/ へ退避する手順に変更したため) |
| 報告書原文 | (r31〜r51) | **欠落**。worktree 内に書かせ `git worktree remove --force` で削除していた |
| 発注書 | `/home/h2suzuki/terminal-configs/drafts/sentinel-review-r{17..52}.md` | 全 36 本が現存。各「## 目的」に前巡の指摘要約と件数推移行があり、r31 以降はここが唯一の一次資料 |
| commit log | `git -C /home/h2suzuki/terminal-configs log -- files/codex_task_sentinel` | 17 巡 (6a83eff) 〜 52 巡 (e243e45) の 62 commit。各 message に直した指摘と変異検証の結果 |
| 対象コード | `/home/h2suzuki/terminal-configs/files/codex_task_sentinel` | 現行版 (selftest 237 tests + 外部 11 tests) |
