...
1. 差分を hunk に分け、追加行を piece に切ります。Markdown は段落ごとに切り、見出しは直後の段落に、コードブロックは直前の段落に付けます。コードは空行で切らず hunk ごとに 1 つの piece にします (関数の途中の切れ端には、判定できる場所の文脈がないため)。どちらも 1500 字を超える分は切ります。見出しで始まる piece は、その新しい節自身ではなく差し込まれた親の節を場所とし、直前の兄弟の節を書き足した内容の説明に添えます。
...
- **配置の問い** (piece ごと、Jev の noul): 書き足した内容は、その節を目的どおりに読みに来た人が求めるものか。合わない例として、保守者向けの規則 (原本ファイル・同期・実行するテスト・ツール間の権限の対応付け)、調べた根拠・確認日・外部文書への注意書き、周りより細かい参考情報、別の見出しに属する内容を具体的に挙げています。コードとテストでは、囲む関数やテストの役目を果たすかを問います。どの関数にも入らないファイルの最上位の行には、そのファイルの最上位に置くもの (import・定数・初期化・新しい定義) かを問います。
...
- **コードで決まることはコードで決める**: "Keep deterministic work in code. It is reliable and cheap." (How to build with System One)、"Direct evidence stays in code." (Autoformat)。形の数はコードで数えます。場所は正規表現と字下げから推し量っていて、構文として確定させていません。構文が壊れたかどうかも Jev の判定に任せています (未対応)。
- **渡した前提はそのまま読まれる**: "`jev-1.13` answers the question you wrote, not the one you meant." (jev-1.13 の癖)。推し量った場所が誤っていると、Jev はその誤った場所で判定します (未対応)。
