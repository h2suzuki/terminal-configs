...
1. 差分を hunk に分け、追加行を piece に切ります。Markdown は段落ごとに切り、見出しは直後の段落に、コードブロックは直前の段落に付けます。コードは空行で切らず hunk ごとに 1 つの piece にします (関数の途中の切れ端には、判定できる場所の文脈がないため)。ただし追加行が途中で別の場所 (関数・テスト・コメントなど) に移るときは、そこで切ります。どちらも 1500 字を超える分は切ります。見出しで始まる piece は、その新しい節自身ではなく差し込まれた親の節を場所とし、直前の兄弟の節を書き足した内容の説明に添えます。
2. 変更前は構文として読めたファイルが、変更後に読めなくなったら、Jev に問い合わせずに拒否します。読めるかどうかはコードで決まる事実だからです。JSON は json モジュール、Python は ast、bash と TypeScript・JavaScript は tree-sitter の構文木で調べます。
3. piece ごとに、ファイルと場所の文脈を機械的に組み立てます (3.3)。
4. 同じ場所に続く piece は同じ問い合わせにまとめます。問い合わせはファイルごとに分け、上限を超える場合だけ同じ場所の中で分けます。
5. 同じサーバーの Jev セッションで判定します。hook はプロセスを起動せず、サーバーが git の差分を読みます。
6. 新しいファイルは piece に切らず、ファイルごとに 1 回問い合わせます。
7. どれか 1 つでも判定が閾値を下回れば、コミットを拒否します (3.4)。

### 3.3 Jev に渡す文脈

Jev の精度は、質問の言い回しより「どういう背景で、どういう種類のファイルの、どの場所に、何を書き足したか」を短く明確に渡せるかで決まります。そのため、次の欄を LLM を使わずに埋めます。コードと JSON の構造 (どの関数・クラス・テスト・キーの中か、コメントや docstring、here-document の中か) は、正規表現や字下げの推測ではなく、tree-sitter の構文木から求めます。Jev は渡された前提をそのまま読むので、誤った場所を渡すと誤った判定になります。

| 欄 | 中身 | 作り方 |
|---|---|---|
| `form.background_of_change` | 変更の背景 | コミットメッセージの件名と本文の先頭 |
| `form.file_kind_and_role` | ファイルの種類と役割 | 題名、冒頭の段落、文書全体の形 (行数・表・箇条書き・コマンドブロック・段落の長さ・外部リンク数を数えて英文にしたもの)。コードとテストではファイル冒頭の docstring か先頭のコメント。どちらも無ければ言語とテストかどうか (例: `TypeScript test file`) |
| `hunks[].place_and_its_purpose` | 書き足した場所とその目的 | 見出しの階層と、それぞれの節の最初の段落。コードでは構文木で囲む関数・クラス・テストの見出し行、JSON では開いているキーの経路。コメント・docstring・here-document・既存のコードブロックの中なら、それも添えます |
| `hunks[].style_of_this_place` | その場所の書き方 | 変更前の同じ節の形を数えた文 (新しい節なら文書全体)。コードとテストでは変更前の定義の一覧 |
| `pieces[].what_the_edit_adds` | 書き足した内容の形 | 追加行の形を数えた文。追加行が別の定義の中に新しい定義を始めるときは、その 2 つを明記します |
...
- **配置の問い** (piece ごと、Jev の noul): 書き足した内容は、その節を目的どおりに読みに来た人が求めるものか。合わない例として、保守者向けの規則 (原本ファイル・同期・実行するテスト・ツール間の権限の対応付け)、調べた根拠・確認日・外部文書への注意書き、周りより細かい参考情報、別の見出しに属する内容を具体的に挙げています。コードとテストでは、囲む関数やテストの役目を果たすかを問います。どの関数にも入らないファイルの最上位の行には、そのファイルの最上位に置くもの (import・定数・初期化・新しい定義) かを問います。docstring の行には文書の問いを使います。
...
- 判定の本体がサーバーに配備されていない
- 構文解析器 (tree-sitter) が実行環境に入っていない
