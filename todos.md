...
### sudoers nopasswd の mode 是正 (要否 H.S. 判断)

Goal: `/etc/sudoers.d/nopasswd` の mode を 0644→0440 にするか H.S. が判断し、是正するなら `files/` source も同期して deploy する。

Exit Criteria:
- [ ] H.S. が要否を判断 (visudo -c が 0644 を warn・0440 推奨)。不要なら本 block 削除、是正なら deploy 先 + `files/` 両方を同 commit で更新。
