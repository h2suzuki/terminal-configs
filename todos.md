...
- [x] 回収スレッド (c)(d) は H.S. 判断で drop し close (2026-06-09): (c) MCP per-host 多重 spawn は root+login の fresh install 成功で実質 OK・別途要れば再起票、 (d) decision-list 陳腐化は block 削除で moot
