...
Exit Criteria:
- [x] 本 repo では close (2026-06-09 H.S. 判断: **別プロジェクトで実施**)。 BM25 lexical 層 (commit c7dbcea) は deploy 済のまま残置。 RAG hybrid (sqlite-vec + local embedding model2vec/fastembed) の調査・実装は移管先で実施。 回収していた未起票スレッド 4 件のうち (c)(d) は installer block へ移設、 (a)(b) は git log へ保全 (本 close commit 参照)
