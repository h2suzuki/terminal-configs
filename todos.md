...
### codex review-nudge を rescue 完了 trigger で再導入 (live 確認待ち)

Goal: codex の plugin-only 化で消えた review-nudge を、 `files/claude_managed-hooks/codex_delegation_surface.py` の SubagentStop hook (agent_type に "codex-rescue" を含む時のみ) で再導入し、 codex-rescue 完了時に「codex 出力を敵対的/受入レビューせよ」と main agent へ案内する。 DELEGATE_MSG (ExitPlanMode 時) の review 言及は残す。

Exit Criteria:
- [x] harness payload schema 確認: SubagentStop に `agent_type` (v2.1.69) と additionalContext 注入 (v2.1.163)、 既存 subagent_gate_warn が tool_input.subagent_type を・voicevox が SubagentStop agent_type を実利用 — 判定可
- [x] hook に SubagentStop review-nudge branch + delegation.json (matcher 無し) 登録を追加、 unittest 6 green (pos/neg) + synthetic payload で発火確認、 commit 33b78a4
