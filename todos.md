...
### 旧 claude_extensions.sh 構築マシンの serena 手動除去

Goal: commit c569f5d 以前の `extra/claude_extensions.sh` で構築した別マシン（候補: Hyper-V guest）から serena を手動除去し、腐った hook 参照（`~/.local/bin/serena-hooks`）が残らない状態にする。

旧スクリプト構築マシンに新スクリプトを再実行しても serena は消えない（remove 行ごと削除済み + `claude_user_settings inject` は append-only）。該当マシンで user / root 両方について以下を実行する:

1. `claude mcp remove serena --scope user`
2. `claude_user_settings remove -` に旧 serena fragment（SessionStart activate / PreToolUse `mcp__serena__*` auto-approve / SessionEnd cleanup、`git show c569f5d^:files/claude_user-extensions.json` で取得可）を入力
3. `uv tool uninstall serena-agent`
4. `rm -rf ~/.serena` と各 repo の `.serena/`

Exit Criteria:
- [ ] 対象マシンの有無を確認（extra/claude_extensions.sh を実行した本機以外のマシンが無ければ、その確認をもって本タスク close）
- [ ] 対象マシンがあれば上記 1-4 を user / root 両 scope で実行し、`grep -c serena ~/.claude/settings.json` = 0 と `claude mcp list` に serena 不在を確認

### WT_SESSION を SSH+sudo で guest root まで伝播

Goal: WSL2 の WT_SESSION を SSH 転送＋sudo 保持で Hyper-V guest の root まで届け、 guest の root claude が "windows-terminal" を検出（Kitty protocol / SGR マウス / 同期描画 ?2026h を有効化）する状態にする。

Exit Criteria:
- [x] repo 3 source 改修・commit 0e7abaa — `files/ssh_keepalive_wtsess.conf` SendEnv / `files/sshd_keepalive_wtsess.conf` AcceptEnv / `files/sudoers` env_keep（いずれも既存 directive に累積。 後続 commit で keepalive 2 ファイルを `_wtsess` 改名・sudoers dest を `/etc/sudoers.d/terminal-config` 化）
...
- [ ] guest へ deploy（`debian12.sh` 再実行 or 手動 install + `systemctl reload ssh`、 dest 改名ゆえ旧 `/etc/sudoers.d/nopasswd` は手動削除）し、 **新規** SSH session で `echo $WT_SESSION` が WSL2 と同一 GUID（SSH 転送 OK）
- [ ] guest で `sudo -i` 後も同 GUID 保持（検証時は /root/.bashrc の placeholder を一時無効化し real GUID と確認）
...
- placeholder fallback は伝播失敗を隠蔽する（壊れても非空→claude が誤って windows-terminal 報告）。 ∴ 検証時は一時無効化が必須
- (任意・既存挙動・本 task scope 外) installer 堅牢化候補: `debian12.sh:153` の `systemctl reload ssh ... || true` は socket-activation 下で no-op（次接続が新 master を spawn し実害は薄い、 `try-restart` 併用が堅牢）／ `debian12.sh` は openssh-server を install しない（guest は SSH 接続前提ゆえ実害なし）
