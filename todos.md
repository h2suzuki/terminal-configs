...
Exit Criteria:
- [x] 対象マシンの有無を確認（extra/claude_extensions.sh を実行した本機以外のマシンが無ければ、その確認をもって本タスク close）— 対象マシン実在 (Debian12 SSH server 等)、 H.S. 申告 (2026-06-12)
- [x] 対象マシンがあれば上記 1-4 を user / root 両 scope で実行し、`grep -c serena ~/.claude/settings.json` = 0 と `claude mcp list` に serena 不在を確認 — H.S. が提示コマンドで全マシン実施済みと報告 (2026-06-12)
