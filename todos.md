...
### 自作癖の抑制機構を、事後の警告から編集前の deny へ作り直す

起票: user 2026-08-23 (「いまの implementation-checkpoint は廃棄しないとかな。部品として
機能していなさそうなので」「今日は破棄だけでも、よし」)

Goal: 「既存部品があるなら使う・再構築は承認・自分で書いてよいのは trivial だけ」という要件を、
書いた後の警告でなく**書く前の deny** で担保する。

**廃棄理由 (実測 2026-08-23)**: 現行の implementation-checkpoint は本 session で 190 行の
直接実装があり codex 委譲 39 件がある状態で**一度も発火しなかった**。抑止条件が「session に
委譲記録が 1 件でもあれば以後恒久的に黙る」ため。さらに正本 (単一 file・追加 10 行以内・
test 追加なし の AND) のうち行数条件だけを、しかも由来のない 50 行という別の数字で見ており、
計測単位も正本の「session 累積」に対し実装は turn 単位。**由来調査の結論 = 要件が代理指標に
すり替わったまま実装され、レビューは発注書との一致だけを見て通した**。

**抑止条件の裏取り (実測 2026-08-23)**: codex 委譲が 1 件も無い session で checkpoint が
**発火した** (直接実装 37 行を検出)。委譲 39 件の session では黙り、0 件の session では鳴る —
上の「委譲記録が 1 件でもあれば以後恒久的に黙る」という診断が両側から裏づけられた。
なお発火した内容自体は正しく、正本 3 条件をどちらの commit も超えていた
(`906ab58` = +24 行・test 追加あり / `18f0c6d` = +13 行・test 追加あり)。委譲不採用の理由は
前者がユーザーの直接指示、後者は誤発火の原因を特定した同 turn 内の是正で、いずれも
発注書往復のほうが高くつくと判断した。**後者は理由の記録を怠っており、正本が要求する
「委譲不採用の理由 1 行」を本追記で事後に満たしている**。

Exit Criteria:

- [ ] 現行 implementation-checkpoint family を削除した (test ごと)
- [ ] 編集前 deny を実装した — 対象 = repo 内 source への Edit/Write/MultiEdit、判定 = この編集で
  正本の 3 条件 (単一 file・追加 10 行以内・test 追加なし) を超えるか (turn 内の既編集は
  transcript から数え、新しい保存先は作らない)、deny の解除 = 委譲するか、委譲しない理由を
  1 行書いてから編集する
- [ ] subagent に書かせる抜け道の扱いを決めた — 現状 Claude subagent の編集は計測にも受け入れ
  鎖にも乗らない。行数では測れないため別設計が要る
- [ ] 配備し、実際に deny が出ることを実測した
...
- [ ] **コミュニケーション規則の hook 強化 — CLAUDE.md は削らない** (2026-08-21 ユーザー決裁:
  ...
  **2026-08-22 ユーザー指示で着手** — (1)(2) を 1 つの family に統合し、上の reminder と
  同じ発注書で codex へ発注済み
- [ ] **発注書 lint の語彙過剰検出を直す** (2026-08-23 実測で起票) — 「状態」「state」「latch」
  等の語が本文にあるだけで別節を要求する検査が、普通の日本語 (「空状態」「〜という状態に
  なっている」) に反応する。本 session の発注書 5 通に返った違反 延べ 14 件のうち **8 件が
  この 1 検査**で、5 通中 4 通が踏んだ。回避のため文章を不自然に書き換えており (「空状態」→
