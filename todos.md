...
- [ ] **live deploy (H.S. の sudo 承認要 — auto classifier が shared-infra 改変として block)**: `claude_md_lint.py` install + managed-settings wiring swap + 旧 .sh 撤去 + writing-code SKILL.md sync → 実機 SessionStart で parity 確認。 deploy まで live は旧 .sh 稼働 (無傷)・repo は .py ゆえ source↔live divergence
