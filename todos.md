...
2. `claude_user_settings remove -` に **serena 3 エントリのみ**の fragment（SessionStart activate / PreToolUse `mcp__serena__*` auto-approve / SessionEnd cleanup）を入力。`git show c569f5d^:files/claude_user-extensions.json` から該当 3 エントリを抽出して一時 json を作る（**全文をそのまま渡すと非 serena の user hooks まで削除されるため厳禁**）
