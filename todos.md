...
### title_icon 修正の deploy + 実機検証

Goal: commit e1bd3fc (Stop 時 background_tasks 判定 + SessionEnd 明示タイトル復元) を `~/.claude/hooks/` へ deploy し、2 バグの解消を実機で確認する。

Exit Criteria:
- [ ] H.S. が `install -m 755 files/claude_user-hooks/title_icon.py ~/.claude/hooks/title_icon.py` を実行し checksum 一致
...
Goal: subagent セッションで skill invoke を検出できず Edit/Write が常に deny される問題 (2026-07-10 実装 agent 2 体で再現、Bash heredoc 編集で回避) の原因を特定し、gate を subagent 対応にする。
