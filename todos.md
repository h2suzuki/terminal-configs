...
- [ ] (c) 起動 helper の要否を判断する — (d) が入ったので独立評価が可能になった。素の `node <長い path>` を書く動機を消す施策で、変数に入れる誘因そのものを減らす。コストは 4 箇所 (実行ファイル + `.sh` 2 本の copy 行 + excludedCommands 登録)。命名と excludedCommands 登録は設計判断を含むため **要相談**。(d) の運用実績を数週間見てから判断してもよい
