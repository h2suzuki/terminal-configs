...
Exit Criteria:
- [x] repo 3 source 改修・commit 502aa9b — `files/ssh_keepalive.conf` SendEnv / `files/sshd_keepalive.conf` AcceptEnv / `files/sudoers` env_keep（いずれも既存 directive に累積）
...
- [ ] guest へ deploy（`debian12.sh` 再実行 or 手動 install + `systemctl reload ssh`）し、 **新規** SSH session で `echo $WT_SESSION` が WSL2 と同一 GUID（SSH 転送 OK）
