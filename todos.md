...
- **回避策**: (1) kill-list 外の model (Opus 4.7 / Haiku 4.5) へ切替で即復活 (issue #76076 repro)。(2) 候補: env `DISABLE_GROWTHBOOK=1` — GrowthBook fetch を止め Qe() が default [] を返せば DX()=false で Opus 4.8 のまま復活しうる (#75577 報告、**自機未検証**、全 flag 無効化の副作用あり)。維持で追跡のみ要るなら todos.md 代替。
