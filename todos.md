...
- [ ] codex を MCP server 登録: `claude mcp add codex --scope user -- codex mcp-server` を claude_extensions.sh (root/login-user) へ追加 + 委譲が `mcp__codex__codex` で matchable と実機確認
- [ ] codegraph-first gate (PreToolUse `Grep`/`Read`): 構造 signal (`type`/`glob`/`path`) 主軸で判定 — 確実なシンボル/コールツリー検索は **deny** (→`codegraph_search`/`explore` か `codegraph_callers`/`callees`/`impact` 誘導)、疑わしきは **advisory** (allow+additionalContext)、非code/out-of-repo は allow。 **freshness gate 無し**。 **detour**: 当turn に同一 target 再試行で allow (read→deny→再read→allow)。 Read は cold 探索のみ deny・targeted は allow。 fail-open
- [ ] codex delegation surface: PreToolUse `ExitPlanMode` で plan→実装 nudge + PostToolUse `mcp__codex__.*` で review/adversarial-review 補強 (deny でなく additionalContext)
- [ ] 両 hook の unittest (deny/allow 条件・fail-open・freshness 境界)
- [ ] deploy (voicevox 方式): `files/claude_managed-delegation.json`→`managed-settings.d/delegation.json` + 新 hook script を `claude_managed-hooks/`、claude_extensions.sh に配線 + files↔deploy 同期 + README
- [ ] e2e 実機: 実装委譲 (`mcp__codex__codex`) → Claude review → `/codex:adversarial-review` 第二意見 の往復確認
