...
### memory surface の精度 (recall & precision) 分析

Goal: UserPromptSubmit (`memory_surface.py`) と Stop (`stop_checks.py` の RAG surface) で出る memory surface の実績を transcript から収集・判定し、recall / precision の現状値と誤り傾向 (無関係 surface / 出るべきが出ない) を定量化する。まず分析、改善は結果を見て別途。

Exit Criteria:
- [ ] surface 実績 dataset 化 (直近 session 群の transcript から surface 発生箇所と対象 entry を抽出)
- [ ] 各 surface の relevance 判定 (precision) と、surface されるべきだった局面の見逃し推定 (recall) を実施
