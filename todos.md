...
調べる前に立てた推論を、 Claude が事実として出力する。 2026-08-29 の session の実測では、
確かめずに書いた断定 6 件が全て実測に否定され、 測って書いたものは 1 件も外れなかった。
「show-me は参考にならない」「再発の上位 2 件で約 6 割」「共有 cache 方式は無い」
「todos.md に記録なし」「hook が存在しない (文字列 grep のみ)」、 そして自作コマンドが
無条件に印字した「出力なし」をそのまま報告した 1 件。
**モデルの内側から検出できないのがこのバグの核心**。 外れている推論は、 当たっている推論と
区別のつかない形で出てくる。 6 件はどれも自信ありげに断定されていて、 保留も hedge も
付いていない。 だから内省では止まらず、 ユーザーが調べて指摘するまで残り続ける。
緩和するには、 発話の前に必ず調査を挟む。 grep は除外ゼロで始める。 走査出力を貼れない
ものに「確認した」と書かない。 推論を先に置くと、 材料はその推論に合うか否かで裁かれ、
材料が持っている答えが見えなくなる。

CAVEAT: 調査前の推論 — 完了判断での再発 (同じバグの別場面)
完了の判断も推論なので、 調べる前に書けば同じように外れる。
2026-08-27、 要件は「surface hook が when: で振り分ける」だった。 私が出したのは「gate が
値域外を拒否する」。 振り分けが動くかを一度も走らせないまま完了として閉じ、 block まで削除した。
その削除した文自身に「stop を含む entry だけ配備」と書いてある。
やるべき調査は簡単だった — `when: after-subagent` の entry を 1 件置いて subagent を走らせ、
surface されるか見る。 それだけ。 やらなかったので導入 (2026-08-27) 以降 surface は 0 回のまま
2 日が過ぎ、 レビューの暴走を止めるはずの教訓が届かず、 6 巡・858 万 token・49 agent を費やした。
緩和するには、 走らせて出力を見ていないものを完了と書かない。 「たぶん動く」「動くはず」は推論。
