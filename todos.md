...
### [Done 2026-06-10] serena install が guest の no-build ポリシーで失敗

真因: Debian 12 guest の **`/etc/uv/uv.toml`（repo 管理外・管理者/base image 由来）に `no-build = true`**。serena 必須依存 pywebview==6.2 → proxy-tools==0.1.0（PyPI に sdist のみ・wheel 無し）の source ビルドが禁じられ `uv tool install` が解決失敗。version 非依存（pin v1.5.3 も unpinned main も同一エラー）。`UV_NO_BUILD` env 上書きは uv 仕様上不可。
対処: `extra/claude_extensions.sh` の serena 2 行に `--no-config` 付与で system uv.toml を無視（他設定は index 等無く影響無）。pin は無意味ゆえ commit be04afb を drop し unpinned に戻した。
検証: no-build 模擬下で v1.5.3 / unpinned main 両方フル install を e2e（exit 0・proxy-tools ビルド・3 executables）。voicevox の pasimple は wheel 有りで無影響。
