...
- [ ] deploy (voicevox 方式): drop-in `managed-settings.d/delegation.json` + hook 2 本を `/etc/claude-code/hooks/` へ deploy 済 (diff -q 一致・mode 755/644)、claude_extensions.sh 配線済 (commit 4e7950f)。 **残: README 追記**
...
- [ ] 旧案 (skill_reminder_gate clone で source Edit/Write を blanket-ish に deny) は本 session で **見送り確定** — patch 反映/fixup の FP 過大ゆえ ExitPlanMode nudge へ置換
