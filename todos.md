...
### Sandbox: 再起動後に rootful docker.sock の connect 遮断を実機検証

Goal: `c0477a8` で deploy 済の Method B (`network.allowAllUnixSockets:true` + `filesystem.denyRead:["/var/run/docker.sock"]` = 全 unix socket 許可・rootful docker.sock だけ /dev/null mask で connect 遮断・UID 非依存) を、新 config を読んだ session で実機確認する。前 session は稼働 session が旧 config snapshot で固定 (socket()=EPERM・docker.sock 未 mask) のため未検証で終わった。

Exit Criteria:
- [ ] 再起動 session の sandboxed bash で `python3 -c "import socket;socket.socket(socket.AF_UNIX,socket.SOCK_STREAM)"` 成功 (allowAllUnixSockets 効)
- [ ] `s.connect('/var/run/docker.sock')` が ECONNREFUSED (denyRead の /dev/null mask で非socket化) → codex の唯一の未実証部分が確定
- [ ] NG なら denyRead 機構を再調査 (codex 結果: bwrap が `--ro-bind /dev/null` で mask、kernel `unix_find_other` が `S_ISSOCK` 偽で ECONNREFUSED)

背景: 本 session で managed-settings に sandbox を段階構築 (enable→strict[failIfUnavailable+allowUnsandboxedCommands:false]→credentials deny→excludedCommands[git/gh/docker/codex/agent-browser/claude --bg/claude agents]→gcloud deny→seccomp helper を base deploy script へ→allowWrite ~/.claude/plugins/data→Method B socket)。全 commit 済 (`ec7d6c0`..`c0477a8`)。seccomp helper (@anthropic-ai/sandbox-runtime) は deploy script 追加済だが当機 live 未導入 (任意・無くても denyRead mask は有効)。
