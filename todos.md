...
### (要相談) Terminal bell 抑止をログインユーザーにも適用

Goal: `files/inputrc` の bell 抑止を root 以外にも効かせるか方針を決め、採用なら installer にログインユーザー向け配置を実装する。

Exit Criteria:
- [ ] H.S. が採否を判断（不採用なら本 block 削除のみ）
- [ ] 採用時: 両 installer に `copy inputrc $LOGIN_HOME/.inputrc`（`--owner` 形式は ubuntu2404-wsl.sh:465-466 参照）を追加し、ログインユーザーで bell 抑止を実機確認
- [ ] 採用時: README.md / README.en.md §1 の「（root のみ）」表記を更新
