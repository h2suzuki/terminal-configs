...
Exit Criteria:
- [x] bash 版の挙動を parity 仕様として列挙 (2026-06-08 workflow wt6xkt7ha が bg-job dispatch / staging→cache / reaper / recursion guard / @-import BFS / noclobber / setsid / cache key を line-cited 網羅)
- [x] Python 実装 `claude_md_lint.py` (name-preserving snake_case、 verb-first lint_claude_mds は afca515 で H.S. drop 済) + 17 embedded unittest + ruff/ty clean。 実 bash と salt-aligned fixture で parity 検証 — cache-key byte 一致・dispatch argv exact (skill_body trailing-\n strip 後)・cache-HIT 出力は内容 byte 一致 (JSON pretty/compact 差のみ・parser inert)。 CACHE_KEY_SALT v4 で旧 cache 無効化、 wiring/contract doc/writing-code 例も .py 同期。 commit 48358bc
- [x] live deploy 完了 (2026-06-08 H.S. 承認後): `/etc/claude-code/hooks/claude_md_lint.py` install + managed-settings wiring swap + 旧 .sh 撤去 + writing-code SKILL.md sync、 source==live parity OK。 実機 SessionStart 検証 — deployed copy で unittest 17/17・cache-HIT 正しく emit・cache-MISS で実 `claude --bg` dispatch (bg job 8a96ebe4 + inflight claim + lock 先行・dispatch-error 無し)。 subagent の非 hermetic test 汚染 v4 cache も除去

経緯: 417 行の複雑な orchestration ゆえ単純 rename でなく本格 port。 Work file: `files/claude_managed-hooks/claude_md_lint.py` (embedded 17 unittest = 回帰 guard)
