...
### fullscreen TUI 乱れ — !use_pty は効果あり / 残存は上流 renderer (2026-06-16)

Goal: sudo -i root の fullscreen TUI 画面乱れ（行ダブルクリック / workflow の行追加で崩れ・Ctrl-L で復旧）を解消する。

根本原因 (本 session 確定): sudo `Defaults use_pty`(/etc/sudoers L16) が中間 pty (pts/5) を挟み、fullscreen の no-flicker 差分描画が desync。env / version / config は消去済（COLORTERM・TERM_PROGRAM は正常側も unset、scorer は同 fullscreen + 単一 pty で正常、/tui default では乱れず＝renderer 固有）。

対策デプロイ済 (commit 5199d41): `files/sudoers` + `/etc/sudoers.d/terminal-config` に `Defaults !use_pty`。visudo -c OK・`sudo -l` で `!use_pty` 後勝ち確認・kernel TIOCSTI=0 ゆえ代償なし。

検証結果 (2026-06-16 本 session・単一 pts 復元後):
