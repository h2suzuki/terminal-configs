...
Exit Criteria:
- [ ] 別端末で interactive session を入力待ち (idle) にし、 `claude agents --json` 出力に当該 sid が現れるか確認 (status 値も記録)
- [ ] 現れる → idle-alive 除外が成立。 SKILL-HOOK-CONTRACT の SessionStart 節に 1 行裏取りメモ追記して close
- [ ] 現れない → fix は busy-parallel のみ有効 (no regression だが idle 未解決)。 別案 (PID-marker 等・本 session で設計済) の要否を再評価

Note: registry 取得失敗時は旧 mtime margin に degrade するため、 どちらに転んでも現状より悪化はしない。 設計議論の経緯と PID-marker fallback 案は commit b5ee241 の会話。
