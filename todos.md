...
- [x] 発注書の雛形を用意した — 検査器が要求する節をすべて空欄として持ち、埋め忘れが機械検査で
  見える形にする (注意文は書かない)。**達成 2026-08-24**: 雛形 3 種を配備し、配備先で
  空欄 13 件が所見に出ることと、埋めれば所見ゼロになることを実測。注意文は 1 行も入れていない。
  **設計完了 2026-08-24 (ultracode・7 agent)**: 判定書 `drafts/order-template-verdict.md`。
  検査器の要求節を全数列挙 (無条件 7 節 + review 条件 1 節 + fix 条件 5 節) し、既存発注書の
  corpus と、雛形を持たない他の成果物種別を棚卸ししたうえで、3 方向の独立設計を判定させた。
  **採用 = 3 案の接合**: (i) 雛形の materialize は hook が行い、著者が選んだ path に骨組みを
  書いてから Write を deny する — 「参照する」という別行為を消す、(ii) 骨組みの生成は
...
- [x] 雛形を経由しないと生成できない形にした — 骨組みを出力する経路を作り、それを起点にする
  (参照が生成の一部になり、別の行為でなくなる)。それでも記憶から書き始めた場合は止める。
  **達成 2026-08-24**: `codex_order_lint --new` が起点。記憶から書いた Write は配備先の
  `codex_order_scaffold` が deny し、同じ path に骨組みを置くところまで実測。
  骨組みは hook が書くので model は未読 → `read_before_edit` が次の Edit を止める =
  雛形を読むことが唯一の進み方になる (参照が別行為でなくなる)
- [ ] 同型の欠落が他に無いかを棚卸しした — 雛形を持たずに毎回書いている成果物の種類を列挙する
- [ ] 効果を実測した — 導入後の発注書で、雛形で防げる類の往復がゼロになったことを確認する。
  **基準値は取得済み 2026-08-24**: 配備先の検査器で drafts 系 303 件を走査し、発注書と判定
  144 件のうち**現行規約で所見ゼロは 12 件**。所見の上位は review-kind 110 / 必須節の欠落 64 /
  fix 節の欠落 47 / 裁定ゼロ・command が fence でない 各 12 / kill-by-port・触らない path 各 8 で、
  **いずれも骨組みが最初から持っている項目**。
  **数字の読み方に注意**: 144 件の多くは規約が増える前の文書で、review-kind は 2026-08-21 追加。
  つまり 110 件は当時の違反ではなく**現行の物差しを過去へ遡及適用した結果**であり、
  「雛形が 110 回の往復を防いだはず」とは言えない。残るのは比較対象 = 導入後に実際に書いた
  発注書の往復回数で、これは次に発注する機会が来たときに取る
...
- [ ] **review 運用の gate 群を実装・smoke・deploy** — (a) 発注書 lint に「既製手段の棚卸し」節
  ...
  **deploy 完了 2026-08-22** (ユーザーが base setup 実行・stop_checks.py / codex_order_lint /
  ...
  設計材料として残してよい)。
  **誤爆実測 9 件と凍結の判断は Medium の「随伴エージェント待ち — モデル判定へ回す案件」
  block へ集約した** (2026-08-25)。3 つの型 (語の境界と用法を見ない / 窓が turn 単位で狭い /
  窓が広すぎ自分の出力を含む) と、モデルに判定させる案はそちらに書いてある。
...
### codex 利用規定の統一 — 受入れ準備までが本 session の scope

起票: user 2026-08-25 (「codex 敵対的レビューは『必ず』ではなくてよいです。緩和する方向で
記載を統一してください」「codex の利用方法について規定しているスキルがあれば、opus ultracode
であらい出して、統一するようにしてください」)
scope 限定: user 2026-08-25 (「どんなタスクを codex に発注すべきかは、別に議論を重ねており、
その成果を持ち込む予定です。持ち込む時は別セッションで新たに作業します」「現時点では、codex に
ついては受入れ準備までやります。スキルなどの編集も、どんなポリシーが来ても良いように
整えてください」)

Goal: 外から持ち込まれる発注ポリシーを、**正本 1 箇所の差し替えで反映できる状態**にしておく。
いまは同じ事柄が skill / docs / hook / memory に分散して食い違っており、ポリシーが来ても
20 組を個別に直すことになる。

**分業 (2026-08-25 決裁)**:

- **発注クラスの中身は本 session の管轄外** — ユーザーが別途議論中で、成果は**別 session**で
  持ち込む。下の D1〜D5 は洗い出しの副産物であり、**採用予定でも提案でもない参考記録**
- 本 session がやるのは**受入れ準備だけ** — 置換対象を文言で特定し、ポリシー依存の記述が
  どこに何組あるかを確定させる。文言の書き換えそのものは行わない

**方針 (2026-08-25 ユーザー決裁)**:

- リグレッションレビューは **opus (max effort)** が担当する。
  **同族になることは許容する (2026-08-25 ユーザー決裁)** — 実装 opus × レビュー opus で
  cross-model 性は下がるが、それを 2 つの条件で補う: (i) レビューは**サブエージェントで
  main session の context を持たせずに**行い、**渡すのは発注書だけ**にする、
  (ii) **effort を高くする**。実装者の欠陥モデルがレビューへ写るのを、添付物でなく
  **context の遮断**で防ぐ形。**本当の cross-model は次段の敵対的レビューが担う**
- **codex の敵対的レビューは必須ではない** — ユーザーの指示時と、高リスク変更
  (auth / data-loss / race / rollback 等) の時に行う
- **実装は codex 固定ではない** — codex に発注すべき実装の種類を定義する
- **適用時期 = ケース 1 完走後** (2026-08-25 決裁「A で」)。ケース 1 は方法論そのものを
  検証しており、走行中に規約を書き換えると測定対象が codex 敵対レビューから opus
  リグレッションレビューへ入れ替わるため。ケース 1 が従う版は base commit `d01ad0a` で固定済み

**判明している食い違い (2026-08-25 実測)**: 役割分担 skill は「codex の敵対レビューは高リスク
変更のときに追加」と条件付きで書き、方法論 §7.3 は毎巡前提で書いている。緩和後はどちらも
条件付きへ揃える。
...
- [x] **緩めない箇所を名指しで除外した** — **完了 2026-08-25**: `docs/codex-usage-donottouch.md`。
  ...
  **要点 2 つ**: (i) `codex_order_lint` の `review-kind` は `none` が既に正規値なので
  「敵対レビューは必ずではない」は **lint を一切変えずに表現できる** — 触ると gate を壊す、
  (ii) 実装も回帰レビューも opus になる回が出るため**兼務禁止の重要度が上がる**
- [ ] **(別 session) 発注ポリシーの持ち込み** — ユーザーが別途議論した成果を受け取り、
  正本へ書き下ろす。本 session では着手しない
- [ ] **(別 session) 統一文案を各 file へ反映し、`files/` と配備先の一致まで確認した** —
  前提は「ケース 1 完走」と「ポリシー確定」の 2 つ

参考記録 (採用予定ではない): 洗い出しの副産物として D1 反復置換 / D2 決定的 gate 検証 /
...
- [x] 発生率を下げる候補方策を、期待効果と実測根拠つきで列挙し提案した — **達成 2026-08-26**:
  `docs/injection-prevention-proposal.md`。**書かせない側の方策だけ** 7 つ。
  全体を貫く観測は「**5 巡連続で欠陥の近因が実装でなく仕様側にあった**」(巡 3・4・6・6・7)。
  内訳 = 契約をコードで書いて渡す / 既存集約という直し方を避ける / 削除を第一選択にする /
  裁定に再現手順を義務づける / 点で直したら一般形を列挙させる / 変更の表面積を報告義務にする /
  受け入れを変異の生存数で書く。
  **弱点も明記**: 最後の 1 つは発生でなく**検出**の方策 (ただし他の効果を測る唯一の指標)。
  既存集約の回避は**相関のみで hunk 遡及が未実施**。ケース 1 では 6 つを同時投入したため
  **個別の寄与は分離できていない**。
  **契約のコード化の限界も実測**: 巡 7 で不正 byte の見逃しと引数の受理域は構造的に消えたが、
  発注側が書き落とした不変条件は散文の時と同じく欠陥になった (読み取り専用の対象で動かない
  退行)。効果は「解釈の相違を消す」ことであって「書き落としを消す」ことではない
...
- [ ] **注入 25 件の hunk 遡及** — 各注入を、それを生んだ fix 所見とその宣言方式に結び付ける。
  「既存集約を含む fix 巡は 5/5 で注入・含まない 2 巡は 0」を巡単位から所見単位へ落とす。
  これが済むまで既存集約の回避は相関どまり
- [ ] **機械検査 3 案の実装** — 名前の実在照合 / 直す前後の判定くらべ / わざと壊すテスト。
  **2 番目 (`claude_ab_probe`) はケース 1 として 7 巡回したが未完成**で、
  commit / merge / 配備いずれも未実施。巡 7 時点の既知欠陥 = 基準版の file 名の乱数が出力へ
...
起票: user 2026-08-25 (「随伴エージェント行きを凍結扱いでまとめて。困りごとと、こうすれば
モデルに判定させられるというアイデアを書く」)
...
エージェントが使えるようになった時点で設計を再開できる状態に保つ。

共通の困りごとは 1 つ — どれも**機械が語や数を見て意図・意味を当てる**形で、当てられない
ことが実測で確定している。凍結中は誤爆と欠落の記録だけ続け、regex の語彙追加・しきい値
調整・新検査の追加は行わない。

#### 1. 自作癖 — 自分で書きすぎたかの判定

困りごと: 廃止した implementation-checkpoint は追加行数という代理指標で測っており、実測で
4 つ壊れていた。(a) session に codex 委譲が 1 件でもあると以後恒久的に黙る (委譲 39 件の
session は 190 行の直接実装を検出せず)、(b) transcript が 2MB を超えて undercount になると
判定式からしきい値ごと外れ 1 行でも鳴る、(c) 正本の要件は「単一 file・追加 10 行以内・
test 追加なし」の AND かつ session 累積なのに、実装は由来のない 50 行を turn 単位で見ていた、
(d) Claude subagent の編集は計測にも受け入れ鎖にも乗らない。

モデルに判定させる案: 行数を答えでなく入力の 1 つに落とし、diff と当 session の発注記録を
渡して「既存部品があるのに再構築したか」「委譲を採らなかった理由が正本の要件を満たすか」を
判定させる。subagent の編集も diff には出るので、同じ経路で (d) も覆う。

#### 2. 発話の意図判定 — warn 系検査 9 種

困りごと: 語のパターンで意図を判定するため、誤爆 9 件が 3 つの型に分かれて出た。

- **語の境界と用法を見ない (5 件)**: 「不明」という語への言及に発火 / 「残り続けます」の
  部分文字列「続けます」に発火 / 「〜には触れません」と「〜に触れません」で助詞だけで反転 /
  第三者 (plugin の code) の動作説明を一人称の宣言と読む
- **窓が turn 単位で狭い (2 件)**: 前ターンで取った根拠や前ターンの Write を見ないため、
  過去形で実在する成果物を指す発話まで空約束として扱う
- **窓が広すぎ、自分の出力を含む (1 件)**: 検問自身の警告文が指摘語をそのまま引用するため、
  1 度発火すると次ターンの入力になり 3 ターン自走した

モデルに判定させる案: 走査対象を当該ターンの assistant 本文だけに絞り、「この文は遂行宣言か・
説明か・引用か」「この主張を支える証跡が当ターンまたは直近にあるか」を判定させる。警告文には
指摘語をそのまま載せない (自走の遮断)。

対象は claim-without-evidence / continuation-claim / hollow-claims / meta-announce-silence /
...
実装に届かない型。引用された識別子の実在照合まではメカニカルに届くが、意味の一致を語彙照合に
落とすと項目 2 と同じ誤爆の道を辿る。

モデルに判定させる案: diff の hunk ごとに「変更された散文」と「変更されたコード」を対にして
渡し、両者が同じことを言っているかだけを判定させる (欠陥の探索でなく一致判定に絞る)。

#### 4. 前提の転移

困りごと: 基礎表 1 件。観測時刻が判定 poll から 1 cadence ずれる類で、変更が暗黙に持ち込む
前提は決定的に列挙できない。

モデルに判定させる案: 項目 3 と同じ経路で「この変更が暗黙に仮定している前提」を列挙させ、
変更後も成り立つかを個別に判定させる。

Exit Criteria:

- [ ] 随伴エージェント (別プロジェクトで検討中) が利用可能になり、上の 4 件の設計を再開
  できる状態になった — それまで作業しない
