...
- [ ] **第1 batch を atomic に実施** (承認済・未着手): dangling_ref_check→check_dangling_refs / handoff_uncommitted_check→check_uncommitted_at_handoff / todos_completion_check→check_todo_completion / push_prompting_check→check_push_prompting。 file rename + wiring json (前3=managed-extensions / push_prompting_check=user-extensions) + deploy (`/etc/claude-code/hooks/`・symlink) + SKILL-HOOK-CONTRACT/SKILL.md/memory/**todos.md 自身**の参照更新 + 各 hook 発火確認
