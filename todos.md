...
- [ ] live deploy (H.S. の sudo 承認要 — auto classifier が shared-infra 改変として block): `stop_checks.py` を `/etc/claude-code/hooks/` へ install → 実機 verify。 deploy まで live stop_checks は旧版稼働 (無傷)・repo は新 family ありで source↔live divergence
