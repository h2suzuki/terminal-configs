...
- [ ] H.S. レビュー承認 → Exit flip + block 削除。 register は ですます に統一済 (commit 9fe0933、 prose 8 行を である→ですます・番号フロー step は体言止め維持)、 SessionStart step2 の述部欠落も修正済 (commit d56b27c)。 残るは H.S. の最終 review (構成/粒度) のみ。 2026-06-08 本 session で skill 一覧 (全 22 entry に category＋≤2文概要)・全 hook の Related 記入・UserPromptExpansion 節 (probe 結果)・Stop の push_prompting_check 欠落補完・応用節 bridge 文を追記し、 hook 記述を 20-agent workflow で実 source 検証して修正 (stop_checks 重複統合・§0 表 block family 4→6) — これらも H.S. review 対象 (commit 20a4858 / 0cf974c)。 **追従要**: 2ca11ff で deny_compound_git_add を broad/pathless add (`-A`/`.`/`-u`/pathless) deny にも拡張したため、 PreToolUse:Bash catalog の同 hook 記述 (現状「compound のみ」想定) を review 時に更新する
...
Exit Criteria:
- [ ] 全 hook の rename map を verb+object で確定 (verb 例: avoid/deny/check/lint/detect/build/...、 gate 系の扱いも決める) → H.S. 承認
- [ ] **atomic に** 実施: file rename + 3 wiring json (managed-extensions/user-extensions/voicevox) + deploy 先 (`/etc/claude-code/hooks/`・symlink) + SKILL.md/SKILL-HOOK-CONTRACT の参照 + 自己参照 CLI (`skill_reminder_gate.py declare` 等) + memory 参照 を同 commit 群で更新し、 各 hook が rename 後も発火することを確認
...
### resume の並行 session 誤選択 (session_resume_context 欠陥1)

Goal: `session_resume_context._find_prior_session` が mtime 最新の他 jsonl を選ぶため、 並行 (background) session が interactive 直前 session より新しいと resume context が並行 session 側を指す問題を、 interactive 直前 session を優先する形へ直す。

Exit Criteria:
- [ ] disambiguation 方針決定 (要相談・design-level): 例 (a) 直近 active 窓内 jsonl を live 並行とみなし除外 / (b) 現 session 開始時刻 anchor で開始後活動の jsonl を除外 / (c) jsonl 内 metadata で interactive vs background 判別 — mtime だけでは原理的に曖昧ゆえ trade-off を verbalize して採択
- [ ] 実装 + smoke + deploy

経緯: 2026-06-08、 欠陥2 (handoff-trim 過剰一致) は commit f32629f で修正済 (👤 user ブロックのみ anchor)。 欠陥1 は同 commit の Known-residual。 並行 session race (本 session で実発生) の resume 面での現れ。 Work file: `files/claude_managed-hooks/session_resume_context.py` (`_find_prior_session` L26-38)
