...
- [x] attribute-existing-issues (H.S. 2026-06-09: (ii) 採択): stop_checks.py に honest-attribution **warning-only** family を新設。 blur phrase (既存のパターン/繰り越し/reasonable default/段階的拡張 等) と wrong-marker (誤/間違/バグ/違反 等) の **60 字近接 pairing** で warn (bare 既存 の FP を proximity で抑制、 v1 known-FP observe-then-tighten)。 4 embedded test + 21/21 unittest + ruff/ty clean + source 動作実証 (blur+wrong→warn / 片方のみ→無 warn)。 commit 45c07fa
- [x] live deploy 完了 (2026-06-09 H.S. 承認後): `stop_checks.py` を `/etc/claude-code/hooks/` へ install (source==live parity・mode 755)。 実機 verify — deployed unittest 21/21、 honest-attribution が blur+wrong→warn(exit0)/片方のみ→無 warn、 既存 family 無回帰 (評価語→exit2・散文 confirm→exit2)
