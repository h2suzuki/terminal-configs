...
### debian12.sh installer 堅牢化（任意・実害は薄い）

Goal: WT_SESSION task の敵対監査 (wf_41c6407f-ecb) が指摘した installer の弱点 2 点を解消し、再 deploy 時の SSH 設定反映を堅牢にする。

Exit Criteria:
- [x] `debian12.sh` の `systemctl reload ssh ... || true` が socket-activation 下でも確実に反映される形（`try-restart` 併用等）になっている（現状は no-op、次接続が新 master を spawn するため実害は薄い） — 6d2f69e が no-op reload 行を削除、99490b5 が `run systemctl restart ssh.service` を追加（ubuntu2404-wsl.sh も同様）
- [x] openssh-server を install しない現仕様の扱いを決定（install 追加 or 前提として script 内に明記） — 6d2f69e が `apt install openssh-server openssh-client` を追加し「install 追加」で決定
