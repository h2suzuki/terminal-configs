...
### ターミナルタブの状態別アイコン (back-port 済・他端末反映と整理待ち)

Goal: タブを見ただけで質問中・プロンプト待ちのセッションを即発見できる状態別アイコンを実現。 v1 (Opus 4.8) は「ビジュアル微妙 + タイトル文字列が壊れる」で H.S. 却下 → v2 (Fable 5・2026-07-03) で全面再設計済。 H.S. 指示で「まず repo 非 commit の実験」→承認後 back-port (commit 4efa99e)。
...
- [x] H.S. が本機で deploy し表示確認 (2026-07-03 環境チェック session): deploy 済 hook が canonical `files/claude_user-hooks/title_icon.py` とバイト一致、settings.json に 8 event 配線 + DISABLE env、本セッションでライブ発火 (state=run・summary/name 捕捉)。 /rename 追従を session entry 実測 (name='アイコン確認'・nameSource=None → resolve_title 採用)。 **H.S. の目視で無印↔💬・❓・⚠️・/rename を確認 (「良いと思います」)**。 他端末は同一 canonical + 同一 deploy script の機械的 rollout ゆえ standing 追跡不要と H.S. 合意
- [x] 暫定配線の整理 (2026-07-03 検証完了): settings.local.json の hooks + env 撤去済、旧 live script `~/.claude/title-icon-hook.py` 消滅・state 残骸 (debug-*.json / nonexistent) も無し。 残る title-icon-state/ の session-id ファイルは正常動作の state で残骸ではない
