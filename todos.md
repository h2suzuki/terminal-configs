...
- [ ] 観測 (v1-FP): 散文 CONFIRM/ROUTING を whole-message 走査するため declarative 文の prose-FP 余地あり → 観測し必要なら `?` anchor / tail-only 走査で tighten (sibling family の v1-FP 同様 observe-then-tighten)
...
- [ ] **`deny_unsafe_git_reset` を PreToolUse:Bash catalog に追記** (2026-06-08 H.S. 指摘・gap 監査で全 24 hook 中**唯一の未記載**と確定。 verb-first で新規追加され rename 統一の発端になった hook。 番号フロー形式 entry + Related を記入)
- [ ] H.S. レビュー承認 → Exit flip + block 削除。 register は ですます に統一済 (commit 9fe0933、 prose 8 行を である→ですます・番号フロー step は体言止め維持)、 SessionStart step2 の述部欠落も修正済 (commit d56b27c)。 残るは H.S. の最終 review (構成/粒度) のみ。 2026-06-08 本 session で skill 一覧 (全 22 entry に category＋≤2文概要)・全 hook の Related 記入・UserPromptExpansion 節 (probe 結果)・Stop の check_push_prompting 欠落補完・応用節 bridge 文を追記し、 hook 記述を 20-agent workflow で実 source 検証して修正 (stop_checks 重複統合・§0 表 block family 4→6) — これらも H.S. review 対象 (commit 20a4858 / 0cf974c)。 **追従要**: 2ca11ff で deny_compound_git_add を broad/pathless add (`-A`/`.`/`-u`/pathless) deny にも拡張したため、 PreToolUse:Bash catalog の同 hook 記述 (現状「compound のみ」想定) を review 時に更新する
...
- **完全 event→hook 対応表**:
  ...
  - PreToolUse: read_before_edit.py(check,Read|Edit|MultiEdit) | check_dangling_refs.py+memory_routing_gate.py(guard)+skill_reminder_gate.py(gate)+comment_rationale_gate.py(Edit|Write|MultiEdit) | avoid_cd.py+deny_compound_git_add.py+deny_compound_git_commit.py+check_commit_format.py(Bash) | subagent_gate_warn.py(Task|Agent) | declare_and_proceed_gate.py(AskUserQuestion) | check_commit_author.py(user,Bash)
  - PostToolUse: read_before_edit.py(record,Read|Write) / memory_routing_gate.py(sync,Write) / check_todo_completion.py(Bash)
...
### hook ファイル名の verb+object 統一

Goal: managed/user hook の命名を verb-first (verb+object) に統一し、 現状の混在 (`*_gate` / `*_check` / `deny_*` / `detect_*` / hyphen 1 例 等) を解消する (2026-06-08 H.S. 提起・スキーム例示: dangling_ref_check→check_dangling_refs / handoff_uncommitted_check→check_uncommitted_at_handoff / todos_completion_check→check_todo_completion / claude-md-lint.sh→lint_claude_mds)。

Exit Criteria:
- [x] スキーム/scope 決定 (2026-06-08 H.S.): verb+object。 gate 系 (skill_reminder_gate / declare_and_proceed_gate / memory_routing_gate / comment_rationale_gate) は `*_gate` **据え置き**。 第1 batch = `*_check`→`check_*` の 4件のみ承認、 noun-first 名 (stop_checks / feature_findings_build / session_cleanup / session_resume_context / memory_surface / subagent_gate_warn / subagent_gate_suggest) は**また今度**
- [x] **第1 batch を atomic に実施** (2026-06-08 完了): dangling_ref_check→check_dangling_refs / handoff_uncommitted_check→check_uncommitted_at_handoff / todos_completion_check→check_todo_completion / push_prompting_check→check_push_prompting。 source git mv 4件 + wiring(managed/user extensions) + self-ref(check_todo_completion) + extra installer copy 行 + SKILL-HOOK-CONTRACT/writing-todos SKILL.md/todos.md 参照を atomic commit。 deploy: managed=/etc/claude-code/hooks/(新 install→wiring→旧 rm)・user=~/.claude/hooks/+settings.json、 全 parity diff 一致・deploy 旧名 0。 発火確認: 4 hook を production stdin path で実行 rc=0、 check_todo_completion は隔離 repo の all-[x] block を検出し新 prefix emit を実証。 memory(feedback_honest_commit_attribution) は退役済かつ「commit d318573 で push_prompting_check.py 追加」という歴史的に正確な記述ゆえ意図的に非更新
- [ ] **第2 batch (noun-first 名)** は H.S.「また今度」で保留 — stop_checks / feature_findings_build / session_cleanup / session_resume_context / memory_surface / subagent_gate_warn / subagent_gate_suggest 等。 承認後に同手順 (git mv + wiring + 全参照 + deploy + 発火確認) で実施

経緯/前提: coupling 実測 (2026-06-08) — `feature_findings_build`=7・`stop_checks`/`claude-md-lint`/`memory_surface`=6・`declare_and_proceed_gate`/`read_before_edit`/`push_prompting_check`/`subagent_gate_suggest`=4 等、参照は wiring json 以外に SKILL.md・doc・自己参照 CLI に波及。 **並行 session を止めた上での専用 atomic タスク**として実施 (途中状態で hook 配線が壊れると全 session に影響)。 新規 `deny_unsafe_git_reset.py` は既に verb-first 準拠。

### claude-md-lint.sh の Python 化

Goal: 唯一の `.sh` hook である claude-md-lint を Python へ port し、 他 hook と言語統一 + embedded unittest / ruff / ty を効かせる (2026-06-08 H.S. 提起「python の方がよいか」→ 複雑度的に妥当と判断)。

Exit Criteria:
- [ ] bash 版の挙動 (bg-job dispatch / staging→cache / reap_inflight・fallback_sweep / recursion guard / @-import BFS / noclobber claim / setsid self-reap / cache key) を parity 仕様として列挙
- [ ] Python 実装 + embedded unittest + ruff/ty + deploy (rename `lint_claude_mds.py`、 wiring json の `.sh`→`.py` 差し替え、 cache key salt bump で旧 cache 無効化) → 挙動 parity を実機確認
