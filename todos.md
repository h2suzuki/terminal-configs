...
### codex review-nudge: SubagentStop approach 無効確定 → 代替経路を再設計 (H.S. 決定待ち)

Goal: codex-rescue 完了時に「codex 出力を敵対的/受入レビューせよ」review-nudge を main agent へ確実に届ける。 当初の SubagentStop hook approach は **2026-06-19 の clean single-session E2E で無効確定** → 同 session で revert 済。 DELEGATE_MSG (ExitPlanMode 時) の review 言及は維持 (PreToolUse は surface する)。
...
Exit Criteria:
- [ ] H.S. が代替経路を決定: (a) 完了時 nudge を諦め DELEGATE_MSG + tool-role-delegation skill に委ねる (gap: plan mode 非経由の直接 /codex:rescue では DELEGATE_MSG 不発) / (b) UserPromptSubmit + session-keyed state で次ユーザーターンに deferred nudge (難点: 発火が遅く main agent が既に動いた後・状態管理が増える) / (c) その他
- [ ] 決定を実装し surface する経路 (PreToolUse / UserPromptSubmit / SessionStart のいずれか) で動作確認

派生元: 2026-06-19 codex plugin-only 化で旧 REVIEW_MSG/PostToolUse 経路を除去 → SubagentStop 版を再実装 (33b78a4) → 本 session E2E で無効確定 → revert。
