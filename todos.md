...
Exit Criteria:
- [ ] H.S. 承認待ち (設計判断): label 専用 2nd regex PUSH_ASK_RE を terse field (label/header) のみに適用、 question/description は PUSH_PROMPT_RE 据置。 候補は scratch で 26 ケース緑 (imperative BLOCK / 「既に push 済み」「push しました」等 factual PASS / question prose に新 FP なし)。 適用時は永続コードゆえ Codex 委譲 + Claude review
