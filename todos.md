...
### session_resume `claude agents --json` の idle-interactive 列挙を実機確認

Goal: live 判定を registry 化した `session_resume_context.py` (commit b5ee241) が、 idle-alive (入力待ち) な interactive session も「終了済」と誤認せず除外できることを実機で裏取りする。 本 session では live session が 1 つ (busy) のみで、 idle interactive が active list に出るかを観測できなかった (発言者: Claude / 承認者: H.S. 起票合意 / status: open)。

Exit Criteria:
- [x] interactive session を idle で起動し `claude agents --json` に出るか確認 — pty で実 interactive claude (session 9637e037) を起動し `kind:"interactive" status:"idle"` で列挙を観測。idle-alive 除外 成立
- [x] 「現れない」反例の検討 — moot (現れた)。 PID-marker 等の別案は不要
- [x] registry の exit 反映 staleness 確認 (H.S. 追加懸念) — kill -9 0.24s / claude stop 0.23s / interactive Ctrl-C×2 0.26s で active から消失。registry は daemon 追跡の live state で staleness 窓なし

Note: SKILL-HOOK-CONTRACT への裏取りメモは同 doc の H.S. review block の範疇ゆえこの検証 task からは外す (検証事実は git 履歴に保全)。設計経緯・PID-marker 案は commit b5ee241 の会話。
