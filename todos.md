...
### stop_checks.py の live 再 deploy (H.S. sudo)

Goal: 編集済 source (commit 4a46944 — intent-without-task gate 追加 + partial_stage nudge 撤去) を live hook へ反映する。 sandbox は no-new-privileges で sudo 不可ゆえ H.S. が host で実行する。

Exit Criteria:
- [x] `sudo install -m 0755 files/claude_managed-hooks/stop_checks.py /etc/claude-code/hooks/stop_checks.py` を host で実行 (2026-07-03 live が source と一致しており実行済と確認)
- [x] source と live の `diff` が空 (一致) を確認 (2026-07-03 実測 SAME)
