...
Exit Criteria:
- [ ] `debian12.sh` の `systemctl reload ssh ... || true` が socket-activation 下でも確実に反映される形（`try-restart` 併用等）になっている（現状は no-op、次接続が新 master を spawn するため実害は薄い）
- [ ] openssh-server を install しない現仕様の扱いを決定（install 追加 or 前提として script 内に明記）
