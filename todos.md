...
- [x] 変数展開でコマンド名を隠す形が両規則を素通りする件の扱いを決めた (2026-07-23) — 4 案を評価し **(d) 実装 + (a) 併存、(b)(c) 不採用**で決着。以下が経緯。live 検証中に**司令塔が probe を短く書こうとして自ら踏んだ**: 直前の Bash 呼び出しで `CC="node .../codex-companion.mjs"` と代入し `$CC task --write` を実行したところ route/tree 両方を素通りし、主 checkout で write task が実起動した (即 cancel、ツリーへの変更なし)。docstring の caveat に列挙済みの既知の限界だが、**敵対的意図なく単に短く書いただけで踏んだ**ため「発注側自身の不注意を防ぐ」という脅威モデルの射程内に入る。変数代入が別の Bash 呼び出しにあると hook からは原理的に見えないため静的解決は不能。検討した 4 案のうち **(d) を採用し (a) を併存**させると 2026-07-23 に決定 (caveat はそのまま維持)。(a) 現状維持のみ = 塞がらない。(b) `node`/`npx` + 引数 `task` の advisory 警告 = 今日の実測 (advisory 8 回・78 回とも読み飛ばし) が効果を反証、deny 化は `node build.js task` 等を巻き込み false deny 面積が大きい。(c) 起動 helper で素の `node` 起動を減らす = 動機は減るが能力は残り、4 箇所 (実行ファイル + `.sh` 2 本の copy 行 + excludedCommands) の追加コスト。命名と excludedCommands 登録は設計判断を含むため (d) 実装後に独立評価する
- [x] (d) codex job 状態による事後検知を実装する — commit f4bfa19、2026-07-23 deploy 済 (canonical と `diff` 一致・mode 755)。`stop_checks.py` の Stop check として実装し、companion の job record (`~/.claude/plugins/data/codex-openai-codex/state/*/jobs/*.json`) を直接読む。条件は `write` かつ `workspaceRoot` が linked worktree でない (= 主 checkout) かつ session 一致。**綴りに依存しない**ため変数展開・コマンド置換・`env -C` を一括で迂回する。job id 単位 latch + worktree-cleanup とは独立の turn latch。state / JSON / git の異常はすべて fail-open。opus 3 lens レビューで 18 件提起・13 件が独立反証を生存 (重複を畳んで 6 項目) を全件修正 — うち blocker 2 件は司令塔の受け入れをすり抜けていた: **symlink 経由の主 checkout が linked worktree と誤判定され検知対象から外れる** (兄弟 hook は両辺 `realpath` なのに `abspath` のみだった)、**turn latch が捨てる前に job latch を焼き新規事故が永久に消える** (turn latch を先に見る順序へ変更)。実 test 8 件 (継承重複 23 件を mixin 化で解消し全体は 111 → 90 件)。**受け入れ時に司令塔が本日の実事故 job で発火を確認**し、deploy 後も deploy 済み hook で再確認 (実事故 job を検知・同 path 2 回目は latch で無音・削除済み worktree の job は除外)。companion の job record schema と status 語彙への依存は docstring に明記
- [x] (c) 起動 helper は**採らないと決定** (2026-07-23) — 新しいコマンド名は判定文字列 `codex-companion.mjs` を含まないため、**両 gate 規則に新名を教えない限り helper 自身が gate をすり抜ける**。機構の面積は減らず同期対象が 1 つ増える。加えて helper が消すはずの動機 (長い path を短く書きたい) は既に縮小済み — `task` は経路 gate で rescue subagent 経由に固定され subagent がコマンドを組み立てるので、人手で短縮したい場面は probe に限られ、その結果は (d) が綴りに関係なく捕捉する。本 session から使う `status` / `cancel` / `result` は write を伴わず gate 対象外ゆえ短縮の利得が無い
