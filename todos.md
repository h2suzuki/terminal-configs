...
- [ ] r77 を発注し、U0 ゼロなら収束 2/2 成立を `docs/sentinel-convergence-log.md` へ記録して
  ...
  数行の縮小修正・新機構不要)。指摘推移 3 → 4 → 1 で乾く方向。**出口はユーザー判断へ**:
  (a) 仕上げ 1 巡 (fix + r80。U0 残なら無条件で bounded-risk 凍結の前提つき) /
  (b) 今すぐ bounded-risk 受入で凍結 (残余 = 同名別 job 記録の併存 × 書込み窓の重なりで
  見張り 1 回が誤報で終わりうる。再起動で回復・頻度は稀)
