...
- [ ] e2e + live 発火確認: codex 委譲 e2e は `/codex:rescue` で実行・Claude review PASS (`drafts/demo_calc.py`、5 tests green)。 **残: (a) hook の live 発火確認 (managed-settings は session 開始時読込ゆえ本 session 未活性、H.S. の /reload-skills か session 再起動が要る) (b) `mcp__codex__codex` 直 e2e (mid-session 登録で未ロード) (c) 高リスク変更での `/codex:adversarial-review` demo**
