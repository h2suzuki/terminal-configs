...
- [ ] **アーキテクチャレビューの段階設計** (2026-08-21 ユーザー指示 → 同日設計確定・doc 反映
  待ち): **巡 0 (実装前)** = 構造のみを 1 巡審査 — 問いは「この部品は要るか / 既存部品・
  記録で代替できないか / 持つ状態を減らせるか」の 3 つ、成果物は構造承認 or 作り直し案の
  判定のみ。判定器は既製 /codex:adversarial-review (plugin 定義自体が design choices への
  挑戦と規定)。5 巡以内の収束保証は巡 0 を通過した構造にのみ適用する。**再発火 trigger** =
  指摘が乾かない (前巡 verdict 必須節に件数推移が残る) / 同 class の層替え反復 (verdict の
  分布読み) / 修正方式の機構追加が 2 巡連続 (lint warn で機械化・gates block (c) 参照)。
  trigger 発火時は fix round を発注せず「この部品を削除したらどの指摘が発生源ごと消えるか」
  の構造巡へ切替。根拠 case = gates 実装で構造 (自前台帳) が誤ったまま 3 巡 18 指摘を磨き、
  「台帳って要るの?」の一問が 3 巡分を無効化 (entry feedback_architecture_before_review が
  doc 反映までの防衛線)
