...
### memory surface hook の RAG + BM25 hybrid 化

Goal: memory_surface.py の検索を BM25 単独から dense embedding + BM25 の hybrid にし、 transcript 由来の評価データで BM25 単独より適切な entry を surface できることを確認して deploy する (DB サーバー等の常駐プロセスは導入しない)。

Exit Criteria:
- [x] hybrid 検索が hook に実装され、 実機 UserPromptSubmit で動作確認済み (unittest 14 件 OK + live 実行 2 件で正答 entry を surface、 latency 33-51ms)
- [x] 既存 memory 全 entry (user + 全 project scope) が embedding 込みで index に移行済み (entries_fts = entries_vec = 16/20/6 で件数一致)
- [x] 追加パッケージ/モデル導入手順が claude_extensions.sh に反映済み (root build + user 共有の 2 block、 同コマンド列を実機実行し完走確認)
- [x] transcript から作った評価データで BM25 単独 vs hybrid を測定し、 改善を定量で記録 (下記サマリ)
- [x] 常駐プロセスなし (SQLite + in-process 計算のみ) を維持 (query 経路は stdlib sqlite3+struct のみ、 venv は --build-model 時だけ)

結果サマリ (実 transcript 1797 prompt → dedup 1341、 手動判定 630 pair: rel 103 / maybe 77 / no 450):
- ranking (n=83): BM25 MRR 0.753 / R@1 0.687 → hybrid (α=0.5) MRR 0.909 / R@1 0.843 / R@2 0.952
- surface 動作点: BM25 現行 floor は発火率 73.8% / 判定済 precision 0.275 / recall 0.699 → hybrid (floor 0.40/0.50 + dense rescue cos≥0.55) は発火率 35.5% / precision 0.337 / recall 0.783。 half-split でも安定
- 方式: potion-multilingual-128M の vocab→fp16 vec を SQLite 化 (`--build-model`、 359MB)、 hook 側は stdlib の Unigram Viterbi + mean pool で再現 (参照実装と cosine p5=0.9998)。 entry 埋込は reminder+keywords

Work file: 実験スクリプト・評価データ・結果 = `drafts/memory-rag/` (gitignored、 結果サマリは本 block に追記)
