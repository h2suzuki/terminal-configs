...
- [ ] **アーキテクチャレビューの段階設計** (2026-08-21 ユーザー指示・compact 後の最初の検討
  事項): 「この構造で本当によいのか?」を問う review を round 0 (実装前) にどう置き、以後
  どの trigger (指摘が乾かない・同 class 反復・fix が機構を足し続ける等) で再実施するかを
  方法論に定義する。根拠 case = gates 実装で構造 (自前台帳) が誤ったまま 3 巡のレビューを
  回し、指摘 18 件を磨いて token を浪費 — ユーザーの「台帳って要るの?」の一問が 3 巡分を
  無効化した。アーキテクチャが誤りのままのレビューは「0 からのやり直しで収束しない」branch
