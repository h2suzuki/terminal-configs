...
- [x] 修正 (agent_id 付き payload は fail-open skip) を canonical に実装し commit `ff1129e`・deploy 済 (deploy 先ハッシュ e3cfda90 一致確認)
- [x] subagent から Edit が通ることを実証 — live subagent で writing-todos invoke → todos.md Edit が ALLOW (2026-07-10 17:28、修正前は同一手順で DENY)、原状復帰済
