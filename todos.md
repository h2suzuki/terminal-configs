...
Exit Criteria:
- [x] sandboxed bash で `socket(AF_UNIX)` 成功 (allowAllUnixSockets 効) — 実機 SUCCESS
- [x] `connect('/var/run/docker.sock')` が ECONNREFUSED — 実機: char-dev(/dev/null mask, inode=4)→ECONNREFUSED。`/var/run` symlink で bwrap 全滅したため `/run/docker.sock` に修正 (fbc8390) し復活
- [x] NG 再調査は不要 (NG せず)
