...
- [ ] **review 運用の gate 群を実装・smoke・deploy** — (a) 発注書 lint に「既製手段の棚卸し」節
  ...
  **誤爆実測 9 (2026-08-24・自己再生ループ)**: communication lint の自己採番検査が、
  **同じ指摘を 3 ターン連続で出し続けた**。本文から該当表現を消しても止まらない。
  **実測で発火源を特定**: 直近 14 行のうち該当を含むのは attachment 3 / system 1 /
  user 2 / assistant 1 で、**モデルの返信文以外が大半**だった。とくに
  **検問自身の警告文が指摘対象の文字列をそのまま引用している**ため、1 度発火すると
  その警告が次ターンの入力になり自走する。さらに workflow の出力と、
  原因調査のための検索コマンド自身も数えられていた (言及と使用を区別しない同型)。
  誤爆実測 4・6 が「窓が turn 単位で狭すぎる」型だったのに対し、これは
  **窓が広すぎ、かつ自分の出力を含む**型。凍結解除時は (a) 走査対象を当該ターンの
  assistant 本文だけに絞る、(b) 警告文に指摘語をそのまま載せない、の 2 点が要る。
  記録のみ。
