...
- [ ] cd 解釈の修正と payload cwd fallback — 実装完了、敵対レビュー中 (worktree `wt-cdgate`)。**当初「`segment[1] == "--"` を採るだけ」と記録していたが、H.S. の指摘を受けた実測で 7 分類の壊れ方があると判明**: flag 誤読 (`--` / `-P` / `-L`)・引数なし `cd`・`~` 未展開・`pushd`・先頭 `VAR=val`・変数展開/コマンド置換・`cd -`。前 5 者は正しく解釈すれば正解が出る。後 2 者は原理的に確定不能なので **payload cwd を実効 cwd とみなして判定を続行** (従来の fail-open を廃止)。fail-open を捨てられる根拠は、同チェーンの `avoid_cd.py` が 2026-07-22 から先頭 `cd` の複合を deny するようになり false deny の面積が縮んだこと。44 test green、司令塔が 16 payload を自ら実行し全件期待通り
- [ ] 発注経路を機械で固定する — `task` サブコマンドを subagent 以外 (本 session の Bash) から起動したら deny し、`codex:codex-rescue` 経由を指示する。`status` / `cancel` / `result` は `codex-cli-runtime` skill が rescue から呼ぶなと規定しているので本 session から許可を維持する。判定は payload の `agent_id` (subagent なら値が入る。`skill_reminder_gate.py:842-843` が同 field で分岐、2026-07-22 実測)。**動機**: 2026-07-23 に司令塔が「forwarder が `--cwd` を落とすかもしれない」という未検証の懸念で正規ルートを外れ、4 回目の直接起動が classifier に deny されて詰まった。rescue 経由では `--cwd` は正しく渡ることを事後に実測。宣言では session 境界で失われるため hook で固定する (`memory-routing` の配置優先順位に従い機械 enforce 可能なものは hook)。既存 parser を再利用するため `codex_worktree_gate.py` に同居させる
