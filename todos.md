...
方針: implementation-checkpoint family には**これ以上手を入れない**。修正も、当初予定していた
置き換え (編集前 deny) も着手しない。要件「既存部品があるなら使う・再構築は承認・自分で
書いてよいのは trivial だけ」は、随伴エージェントが利用可能になった時点でその仕組みの上に
設計し直す。行数という代理指標を磨く方向は、下記の実測が示すとおり筋が悪い。

**凍結時点で判明している欠陥 (すべて実測 2026-08-23・再開時の材料)**:
...
本 session の直接実装 2 件の委譲不採用理由 (正本が要求する記録): `906ab58` はユーザーの
直接指示、`18f0c6d` は誤発火の原因を特定した同 turn 内の是正で、いずれも発注書往復のほうが
高くつくと判断した。

Exit Criteria:

- [ ] 随伴エージェント (別プロジェクトで検討中) が利用可能になり、本 block を再開できる状態に
  なった — それまで本 block の作業は凍結
