...
- [x] Step 3 の二段階退役 — 2026-07-22 完了。正発火は受動観測を待たず **過去 transcript の走査で実測**した (5-agent workflow、hook source から emit 文言を確定してから全 session transcript を検索し、hook 注入 attachment に束縛された実 turn のみを採用、hook source / test / 引用は除外): edited-executable-not-run 2 件、ui-edit-without-screenshot 6 件、pixel-diff-detected 4 件、sandbox_server_gate 7 件。4 entry を OLD-MEMORY 移動 + `Covered by:` footer + `--delete` し、`entries_fts` / `entries_vec` の両テーブルで残存 0 件、本文は footer 1 行追加のみ (backup と diff) を確認。pixel L4 の cover 元 `memory_surface.py` は user hook だが installer が `/etc/claude-code/skel/hooks/` へ配る org 配布物なので「別環境で参照解決されない」除外理由に当たらないと判断
