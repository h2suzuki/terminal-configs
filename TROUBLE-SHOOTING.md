...
> **注意**: `--loopback` 導入直後は**プロセスは正常終了するが音が出ない**問題があった。proxy (module-tunnel-sink-new) の既定 prebuf ≈ 2 秒に対し、音声が短い (例: 1.62 秒) と閾値未到達で永久に再生されないまま close される。`pasimple.PaSimple(..., prebuf=0)` を指定して再生を即時トリガするよう修正済 (WSLg direct は閾値未満でも再生するため影響なし)。
