...
このリポジトリは `ubuntu2404-wsl.sh` / `debian12.sh` および opt-in な `extra/*.sh` (voicevox.sh, signoz.sh 等) の `copy` 行で `files/` 配下の source を `/etc/claude-code/`・`~/.claude/`・`/usr/local/bin/` などへ deploy する構造である。 **canonical source は常に `files/` 側であり、deploy 先はその出力**。
...
今後の作業予定および履歴は `../todos.md` に記録をとる。session 開始時に Read して、次の着手項目を決める。
...
ユーザーとのコミュニケーションを円滑にするスキルが多数登録されている。これらスキルを最大限つかうこと。ユーザーから、いまスキルを使った？と質問されたら既に悪印象。
